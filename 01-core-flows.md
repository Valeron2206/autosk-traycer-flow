...
Recovery: valid crash-tail is byte-verified then authority+nonce heads commit before projection; invalid tail had no effects/nonce reuse. Missing committed bytes fail-closed. Dependency/intent heads protect projections. Signer/secure state must be in a preflight-proven separate OS boundary or hardware enclave inaccessible to model accessibility/ptrace/keychain; no boundary, headless или unpinned project blocks model launch. Проверяет это `security.signer_boundary` в `autosk-flow doctor`: объявленный endpoint, недостижимый из процесса, в котором работает модель, плюс сообщённая демоном идентичность подписи вне этого процесса. Проверка не доказывает изоляцию и этого не утверждает; необъявленная граница — `unverifiable`, а `workflow-preflight` требует эту проверку для implementation и panel, поэтому «нет границы» останавливает запуск модели механически, а не на словах. Что именно останавливается, названо точно: **необъявленная граница и провалившийся зонд**. Пройденный зонд означает «объявленный endpoint отсюда недостижим и демон сообщает отдельную идентичность подписи» — и не означает, что пути не существует. Установить принудительность могла бы только платформенная аттестация изоляции процесса (sandbox profile, entitlements, seccomp/LSM-профиль), которую подписывает не тот же демон, чью границу она подтверждает; её здесь нет, и она названа как отложенная, а не как достигнутая. Утверждать иначе значило бы совершить ровно ту переоценку, ради предотвращения которой граница и существует.
...
Порядок между этим classifier'ом и `docs/contracts/artifact-registry.md` не оставлен на усмотрение: **реестр решает**. Путь, которым управляет класс реестра, получает lifecycle этого класса, а path-role classifier к нему не применяется; путь, которым не управляет ни один класс, отвергается реестром как `unregistered_artifact` и не получает самого дешёвого lifecycle по умолчанию. Отсюда следует, что в поставляемой системе четыре из пяти значений недостижимы: `canonical_artifact`, `governance_mapping_sidecar`, `ordinary_implementation` и `unknown` соответствуют классам реестра, и решает класс. Пятое, `additional_normative`, вместе с park-причиной `artifact_mapping_required` остаётся достижимым и нормативным — но не через этот classifier, а через `unregistered_artifact`: путь с нормативным содержанием, которым не управляет ни один класс, отвергается реестром, и это тот же отказ, названный по-другому. Правило, одновременно нормативное и недостижимое, оставило бы эти park-классы без производителя, поэтому здесь названо, какое именно из пяти значений остаётся, и чем оно производится. Два правила, у которых нет порядка, — это способ, которым один и тот же файл получает два разных gate'а в зависимости от того, кто его читал.
...
Раздел семей — не соглашение об именовании: `resources/panel-roster/family-partition.v1.json` называет семьи и принадлежащие им модели — семейство выводится из модели, а не из харнесса, потому что один харнесс может обслуживать два семейства, — а `validate:provider-preflight` отвергает раздел, в котором четыре обязательных маршрута не покрывают четыре разные семьи. Четыре места в трёх семьях — это трёхмодельная панель под четырьмя именами, и без записанного раздела её нельзя было отличить от настоящей. Lead обязан быть другой семьёй относительно всех авторов и всех агентов, реально исправлявших артефакт. Для mixed authorship применяется мастер-порядок GPT, затем Kimi, Grok, Opus, отфильтрованный до семей вне полного author/fixer set. Тот же мастер-порядок и та же фильтрация действуют для code review в §6: два разных порядка означали бы, что один и тот же author/fixer set даёт разных законных рецензентов в зависимости от того, какую таблицу прочли. Там, где §6 перечисляет меньше семей, это следствие фильтрации по конкретному author set, а не второе правило. Если такой семьи нет, процесс переходит человеку.
...
Мастер-порядок здесь тот же, что в §3 — **GPT, Kimi, Grok, Opus** — и таблица является его применением, а не вторым правилом: из порядка вычитается union авторов и чинивших.

| Union авторов и чинивших | Порядок reviewer |
| --- | --- |
| Claude | GPT, затем Kimi, затем Grok |
| Codex (то же, что GPT) | Kimi, затем Grok, затем Opus |
| Grok | GPT, затем Kimi, затем Opus |
| Kimi | GPT, затем Grok, затем Opus |
| Human/outside | GPT, затем Kimi, затем Grok, затем Opus |
| Любой другой union | мастер-порядок минус каждая семья, входящая в union |
...
1. approved Ticket deltas применяются к приватному staging ref `refs/autosk/epics/<epic_ref_key>/staging` от записанной target base; пользовательская ветка не двигается. Ключ здесь тот же `epic_ref_key`, что и у planning/candidate refs под этим же helper-owned префиксом, — не display ID и не пользовательский slug: одно пространство имён с двумя соглашениями об именовании оставило бы интеграционно-критичный ref без привязки к проекту и без гарантии допустимости самого имени;
...
5. принимается **точная проверенная staging identity**; приёмка относится к идентичности, а не к намерению её произвести. Решение здесь принимает человек — либо оно уже принято им раньше: подписанный `IntegrationAuthorizationRecord` называет финальное дерево, и закреплённая auto-policy на этом шаге **сверяет**, что произведённая identity — та самая, которую пользователь уже связал, а не принимает новую. Самостоятельно принять она не может: §2 требует пользователя для integration-решений, преамбула этого раздела говорит то же, §9 запрещает project alignment policy, а `integration_authorization_policy_issued` отвергает запись, выданную политикой. Policy, принявшая identity, которой пользователь не видел, — это не policy, а умолчание;
...
10. epic переходит в done только после шага 8 и непроблемной очистки шага 9 — на том пути, где ветку двигаем мы. Когда профиль отдаёт финальное движение наружу (шаг 6), шаги 7 и 8 не выполняются, и done по ним не наступает: epic ждёт completion predicate профиля (`docs/contracts/delivery-profile.md` §6a) и переходит в done, когда тот выполнен на точном staging identity; невыполненный или непрочитанный predicate — `completion_predicate_unmet` и human. Отсутствие терминального состояния для этого пути означало бы, что Epic, доставленный через PR или merge queue, не завершается вовсе.

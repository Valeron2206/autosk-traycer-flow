...
Порядок между этим classifier'ом и `docs/contracts/artifact-registry.md` не оставлен на усмотрение: **реестр решает**. Путь, которым управляет класс реестра, получает lifecycle этого класса, а path-role classifier к нему не применяется; путь, которым не управляет ни один класс, отвергается реестром как `unregistered_artifact` и не получает самого дешёвого lifecycle по умолчанию. Отсюда следует, что в поставляемой системе пятизначный classifier недостижим: он остаётся описанием того, какой lifecycle подразумевался для роли, пока её не внесли в реестр одной записью, и исчезает вместе с runtime issue #14. Два правила, у которых нет порядка, — это способ, которым один и тот же файл получает два разных gate'а в зависимости от того, кто его читал.

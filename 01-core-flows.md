...
Единственное исключение: Ticket с correction append'ит immutable `anchor_correction` event в comments родительского Epic, записывает собственный waiting receipt, сначала подтверждённо паркуется human и только затем exact-unblock'ит parent. Ticket никогда не пишет Epic metadata. Crash между park и unblock оставляет parent blocked и возобновляется через `complete_anchor_handoff`. Parent deterministic gate как единственный writer consume'ит events в pending_anchor. При code-only impact rebuild заканчивает все semantic anchor/binding writes и переходит в `resume_repaired_tickets`; только этот следующий step сначала resume'ит human Ticket, подтверждает новое состояние, затем восстанавливает его exact blocker. После resume parent пишет только монотонные фазы открытой recovery operation и закрывает её при входе в ticket_join. Такой порядок оставляет parent повторно запускаемым при crash до blocker и уже запущенный child — владельцем прогресса при crash после blocker. Crash после нового event не теряет его из-за append-only inbox. Planning dispatch также завершает replacements до resume matched humans. Потерянный receipt или event hash блокирует процесс.
...
| Affected done/cancel/new/missing Ticket, code-only repair | rebuild_anchor | consume correction events; old task исключить, superseded_by записать, replacement подготовить; Ticket resume выполняет отдельный следующий step |

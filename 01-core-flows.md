...
До issue #14 любой created/changed behavior-defining document outside the four canonical artifacts is a non-normative mirror only when it contains exactly one closed governance-mapping block to `brief|core_flow|tech_plan|tickets` and references only current decision IDs from that named manifest. Deterministic freeze guard binds path/content/mapping proof into controlling anchor. Missing, ambiguous, stale or extra normative content parks `artifact_mapping_required`; panel/PASS/candidate mint запрещены until decision is expressed and aligned in the named lifecycle.

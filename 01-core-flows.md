...
До panel/waiver host создаёт `refs/autosk/epics/<epic_ref_key>/candidates/<candidate_identity>` на frozen snapshot commit и проверяет полную commit/tree/blob closure. Этот `candidate_keepalive` защищает candidate от Git GC даже после удаления review worktree. После panel/narrow verdict `record_artifact_pass` ещё не завершает kind: он записывает `publication_status=recorded_unpublished` и immutable `planning_publication_op`, связанный с verified keepalive. Детерминированный host step `publish_artifact_pass` создаёт single-parent commit с exact candidate tree, CAS-продвигает planning ref от ожидаемого parent, перечитывает ref/commit/tree/identity, проверяет closure и только затем ставит publication `verified`. После этой проверки одна атомарная helper-транзакция создаёт `refs/autosk/epics/<epic_ref_key>/audit/candidates/<candidate_identity>` на reviewed snapshot commit и удаляет live keepalive по exact old OID; audit ref сохраняется до approved retention expiry. Только после release receipt workflow возвращается в `select_next`. Поэтому recorded PASS не является завершённым артефактом до Git publication.
...
| planning_candidate_keepalive_invalid | freeze_artifact, rebuild_anchor, synthesize_panel, narrow_review_join, record_artifact_pass, current publish step or cleanup according to recorded candidate/operation state | exact candidate keepalive ref/create or audit/release receipt and complete object closure restored or candidate explicitly superseded; cleanup and publication remain blocked until namespace inventory matches metadata/history |

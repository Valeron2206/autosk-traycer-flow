...
`UserDecisionRecord` создаёт только autoskd после signed user-presence challenge. Project public-key pin создаётся trusted client при project init до model task и write-once; workflow TOFU/re-pin запрещены. Rekey требует old+new signatures; lost-key recovery void'ит approvals. Daemon canonicalizes и сохраняет exact signed challenge bytes `{project_hash,record_id,nonce,expires_at,request_id,epic_id?,task_id?,anchor_version,subject_hash,payload_hash,previous_secure_head_hash,journal_sequence}`; signature считается над domain separator + этими bytes. Client показывает тот же packet и подписывает non-exportable OS user-presence key. Daemon проверяет signature/nonce/expiry/одноразовость, append'ит journal, CAS-обновляет secure head и durable consumed-nonce set; только после head commit возвращает record и materializes projection. Workflow side effect до этого запрещён.

Recovery: journal ahead secure head допускается только как contiguous valid signed crash-tail от exact committed head; daemon повторно проверяет stored canonical challenge bytes, затем сначала CAS-forward'ит head/nonce state и только после этого публикует projection. Invalid/replayed/non-contiguous uncommitted tail никогда не имел workflow effects: он quarantine'ится и отсекается до secure head без освобождения nonce. Secure head ahead/missing record, shorter committed prefix, deletion или regressed sequence fail-closed как `authority_journal_truncated` и восстанавливается только exact committed bytes либо destructive user-presence reset. Отдельные protected `dependency_head` и Epic `intent_head` защищают current dependency projection и ordered user-instruction/correction stream. Прямая запись/удаление project projections не проходит reconciliation. Residual assumption — sandbox не имеет signer/secure-head/accessibility/ptrace доступа несмотря на общий UID. Headless/unpinned project не запускает model workflow.

...
| Affected done/cancel/new/missing Ticket, code-only repair | rebuild_anchor | consume correction events; old task исключить, superseded_by записать, replacement подготовить/enroll'ить с blockers; следующий step resume'ит только отдельно сопоставленные human recovery Tickets |

...
Not every declared field has a reader, and claiming otherwise is the kind of sentence this epic has already paid for. These do not: `views` (rendered by slice 3, read by nobody at runtime), `entry_steps` (section 2), `guards[].authority` and `predicates[].reads` (they say who may take a transition and what state it inspects — both are the evaluator's business, and this factory does not evaluate), and `recovery[].required_state` (a design-time statement the graph validator checks). `graph_reasons` is pinned by the schema to two codes that are their own names, so reading it back would only compare a constant with itself.
...
## 6. Caps hold against the durable count

The document counts the taking of one named transition. The engine's durable counters are two, kept in the same write as the position: `metadata.step_visits[step]`, entries into a step by any edge, and — as of patch `0034` — `metadata.transition_takings[from][to]`, the takings of one edge.
...
The ticket that owned the enforcement ships it here, over the measurement that was already durable: a cap's `limit` is read where the document is indexed — the counted edge's guards are bound to below-the-limit, and the guards of the sibling edges carrying its `park_reason` are bound to at-it — and both decision sites conjoin the caller's answer with that term. The term can only narrow: a refused predicate stays refused however the count stands, a predicate wrongly admitted still loses the counted edge at the limit, and an admission past the limit is refused whether the flow selects it or a running session asks for it by name — the explicit path being `ctx.transit({step})`, which `SessionRuntime.transit` hands to the veto this factory builds. The two stops are not the same reason: `review_cap` is recorded when selection takes the carrying sibling, while an explicitly requested counted target refused at the limit carries its own guard's `park_reason` — `planning_candidate_keepalive_invalid` and `project_boundary_invalid` on the shipped document — per section 5's rule that a refused named target answers with the refusing guard's reason. The shapes the per-guard binding cannot serve are refused rather than answered wrong on every decision: a counted edge declaring no guards has nothing to bind the below-limit term to, and a cap with no sibling carrying its `park_reason` has nothing to park on at the limit — both `cap_binding_incomplete` — while a bound guard a second edge also references would constrain a move the cap never named, and an edge two caps bind would owe both limits — both `cap_binding_ambiguous`. The design validator refuses the same four shapes as `graph_cap_binding_incomplete` and `graph_cap_binding_ambiguous`, and a cap counting a transition the document does not declare is refused at build with `transition_not_declared`, the code an undeclared edge already produces. An operator's `resume` is not that explicit path — `Engine.resume` refuses `work` before any hook runs — and a resume after a cap park starts from the `human` status step, so the taking is counted `human -> <target>`: the cap's own pair stays at its limit, and the number of extra rounds cannot be read back out of the cap's counter. The residual the ticket names stands: there is no decision-record format, and until one exists the cap is absolute. The document keeps declaring the caps and they stay inside the digest. The counter being keyed by the `from -> to` pair rather than by transition id is what makes the design validator's second cap check necessary: a counted edge that shares its pair with another transition would have that edge's takings counted toward the cap as well, so `graph_cap_transition_shared` refuses the document — alongside `graph_cap_transition_unknown`, which keeps checking that each `counted_transition` names a real transition. The alternative this section originally weighed — counting takings in the workflow's own state — stays refused for the reason it always was: a count outside the position's write double-counts or loses a round across a crash.
...
`store.setPosition` bumps `metadata.step_visits[step]` — and, when the move traverses an edge, `metadata.transition_takings[from][to]` — in the SAME `mutateTask` write as the position, and `taskStore.writeTask` is one `atomicWrite` of `task.json` — a pid-scoped temporary, an fsync, a rename. The whole record lands in that one write or none of it does.

So the boundary is not a lost count against a moved position, which cannot happen. It is a repeated transition: a crash between `onTransit` returning and that write landing loses the whole transition, so the task stays at the old step with the old counts and the step is entered again. **Execution of a transition is at-least-once; counting is exactly-once per durable entry.**

`scripts/verify-autosk-visits.mjs` stands on exactly that boundary, against a real daemon running a workflow this factory built, and the compatibility workflow runs it on every pull request. `scripts/verify-autosk-takings.mjs` stands on it for the edge counter: it kills the daemon inside the same window and shows a count kept in workflow state double-counting on retry while `transition_takings` stays right.
...
Closed set: `cap_binding_ambiguous`, `cap_binding_incomplete`, `guard_unknown`, `park_reason_ambiguous`, `predicate_unknown`, `status_unknown`, `step_unknown`.
...
`cap_binding_incomplete` and `cap_binding_ambiguous` are the sixth and seventh, and like `park_reason_ambiguous` they refuse under-specified documents rather than malformed ones. Section 6's binding is per guard, and four shapes cannot be bound at all: a counted edge that declares no guards gives the below-limit term nothing to attach to, and a cap whose counted step has no outgoing edge carrying the cap's `park_reason` has no edge to park on at the limit — both incomplete — while a guard the binding reaches that a second edge also references would constrain a move the cap never named, and an edge the binding reaches from two caps would owe both limits — both ambiguous. The design validator refuses the same four shapes; the factory refuses them where it refuses every declaration it cannot honour: when the document is read.

Four further codes the factory produces are owned and closed by `docs/contracts/workflow-graph.md`, which owns what the graph says about itself: `graph_digest_stale`, `no_transition_reason`, `resume_target_not_permitted` and `transition_not_declared`. They are produced here and declared there, and the suite asserts that each of the eleven is closed by exactly one contract.
...
- a cap binds exactly its counted edge's guards and the carrying siblings' — `guard_237`, `guard_238`, `guard_449` and `guard_450` on the shipped document — the counted edge admits below the limit and parks with the cap's own reason at it, the term narrows a caller's answer and never widens it, the veto refuses a requested taking at the limit, an absent, unreadable or array-shaped count reads as none, prototype-named steps are read by own key, a cap counting an undeclared transition is refused `transition_not_declared`, and the four shapes the per-guard binding cannot serve — a guardless counted edge, a cap no sibling carries the `park_reason` of, a bound guard a second edge references, and an edge bound by two caps — are refused at build
...
A further measurer installed the same way, `scripts/verify-autosk-cap.mjs`, stands on the cap's own boundary: it drives a review/fix cycle whose counted edge is drawn at the lowest priority, so an unenforced cap is the durable count passing the declared limit with the task never parking `review_cap` — the defect as it was measured — while an enforced one parks with the cap's reason at the limit and the counted edge is never taken past it.

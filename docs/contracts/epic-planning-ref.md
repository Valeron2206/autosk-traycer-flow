...
The helper journal has closed phases `request_committed -> refs_committed -> receipt_committed`. It fsyncs the exact request and consumed nonce before preparing refs, records and fsyncs the atomic ref/reflog observations after commit, then records and fsyncs the response before return. The wire Schema represents every prefix; final golden exchanges pin `fsync_order=[request,refs,receipt]` and all phase receipt hashes. Lost response replay submits byte-identical request bytes and returns the journaled receipt; `request_committed` may safely execute once, `refs_committed` reconstructs the response from recorded observations, and `receipt_committed` performs zero ref writes. A changed body under the same ID/nonce is rejected. Process death releases `flock`; recovery never removes a guessed lock file. The helper, never the extension host, executes every protected `update-ref`.

Pre-commit rejection uses terminal journal phase `not_applied` with `fsync_order=[request,receipt]`, no refs-commit hash and a value-bound response. Reasons are closed: `expected_old_mismatch` maps to `planning_ref_foreign_movement`; `packed_refs_drift` and `authorization_invalid` map to `planning_ref_capability_missing`. `not_applied` proves zero ref/reflog side effects and cannot be consumed as a committed helper receipt.

...
The helper protocol is action-discriminated. `resources/planning-publication/ref-custody-helper-contract.schema.json` and its example are only the protocol-shape/version pin; their field-set digests are named contract vectors and are never runtime transaction evidence. Actual messages are governed by `resources/planning-publication/ref-custody-helper-wire.schema.json` and six concrete exchanges in `ref-custody-helper-wire.example.json`. A request body closes `{schema,request_id,action,project_root_sha256,epic_ref_key,operation_id,candidate_identity,custody_generation,policy_digest,nonce,packed_refs_sha256,expected_update_message,reflog_producer,reflog_checkpoint,ref_updates}`. `body_sha256 = SHA-256("autosk-flow/ref-custody/request-body/v1\0" + canonical JSON body)` excludes its own digest and authorization. `ref_updates` is an ordered array of exact `{operation:verify|update|delete,ref,expected_old_oid,new_oid}` values, so multi-ref release retains separate planning verify, audit create and live delete observations.

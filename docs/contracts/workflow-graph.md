...
A cap's predicates are read off its edges: the guards on the counted transition, which let the flow take another round, and the guards on the sibling edge that carries the cap's `park_reason`, which the flow takes when the limit is reached. What their descriptions compare with `cap` is the quantity the cap fires on, and that quantity must be one `reads` declares — the union of every predicate's `reads` is the only vocabulary that names what a predicate inspects — or the document is refused with `graph_cap_quantity_undeclared`. The shipped document names it `transition_takings`, the durable count of the counted pair's takings that the runtime counter maintains.

The comparison is `operand OP operand` — the word `cap` on one side and an identifier on the other — each operand either bare or wrapped in one balanced pair of parentheses, whitespace free, `OP` one of `<`, `>`, `<=`, `>=`, `=`, `==`, `!=`, `≤`, `≥`, `≠`. Identifier is read at identifier boundaries: a maximal run of `A–Z`, `a–z`, `0–9` and `_` is one token, and it is an identifier only when it starts with a letter or `_` — a run starting with a digit, like `0transition_takings`, is not an operand, and no `cap` word exists inside it, so `0cap` names nothing and owes nothing. That is the whole grammar. Chains are not supported: a `cap` whose comparison touches another operator on either side is refused outright, never checked on either side — `transition_takings >= cap > round` would otherwise hide an undeclared quantity behind a lawful first comparison. An unbalanced or stray parenthesis inside or beside the comparison is refused with it. A predicate that never names `cap` owes no comparison — the auxiliary guards on those edges are left alone. Every `cap` mention on a cap's predicate must sit inside exactly this form: a mention outside it is a form the validator cannot read and is refused with `graph_cap_quantity_undeclared` rather than skipped, the way `(round) >= cap` and `round ≥ cap` once slipped through.
...
Slice 5 built that runtime, and `docs/contracts/workflow-factory.md` is where it is specified. All three are produced by `src/host/workflow-factory.mjs`, which closes seven further codes of its own. What that changes about `no_transition_reason` is only which half is checked where: the schema still makes it unreachable from a valid document, and this contract's own suite is what shows that; the factory's suite shows that a document which does reach the runtime without the field parks with the code rather than with `undefined`.

Five design-time codes are the cap's own, and they split into what the count is and whether it can be bound. `graph_cap_transition_unknown` and `graph_cap_transition_shared` keep the count attributable: the counted transition must exist and must be alone on its `from -> to` pair, because the runtime counter is keyed by the pair and cannot tell two edges apart. `graph_cap_quantity_undeclared`, covered in section 7, keeps the quantity the cap fires on a declared `reads`. `graph_cap_binding_incomplete` and `graph_cap_binding_ambiguous` keep the per-guard binding servable: a counted edge that declares no guards has nothing to bind the below-limit term to, and a cap no sibling edge carries the `park_reason` of has nothing to park on at the limit — both incomplete — while a guard a second edge also references would carry the cap's constraint onto a move the cap never named, and an edge two caps bind would owe both limits — both ambiguous. The factory refuses the same four shapes at build as `cap_binding_incomplete` and `cap_binding_ambiguous`, so a document that never met the validator still cannot bind wrong.
...
- a cap whose counted transition declares no guards, and a cap no sibling edge out of that transition's step carries the `park_reason` of, are both refused, because the per-guard binding has nothing to attach the below-limit term to and nothing to park on at the limit
- a guard the cap binding reaches that a second edge also references, and an edge the binding reaches from two caps, are both refused, because the term would constrain a move the cap never named or owe two limits
- a cap predicate comparing with a quantity no predicate's `reads` declares is refused, because the quantity a cap fires on is state the document must name — a cap's predicates being the guards on its counted transition and on the sibling edge carrying its `park_reason`, and the union of `reads` being the only vocabulary that names what a predicate inspects — while the same comparison on a predicate behind no cap edge is accepted, the rule binding the cap's own quantity and not every word a description may use; a `cap` mention on those predicates outside a readable comparison — a chain, an unbalanced parenthesis or a bare mention — is refused too, one balanced pair of parentheses and `≤`, `≥`, `≠` being supported notation, so an unreadable writing cannot pass silently
...
| caps | `caps[]` | every `counted_transition` names a declared transition, that transition is the only one on its `from -> to` pair, every `cap` mention on the cap's predicates sits inside a comparison `quantity OP cap` whose quantity `reads` declares, and the per-guard binding is complete — the counted edge declares guards and a sibling edge carries the cap's `park_reason` — and unambiguous — no bound guard is referenced by another edge and no edge is bound by two caps |

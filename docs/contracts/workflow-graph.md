...
| caps | `caps[]` | every `counted_transition` names a declared transition, that transition is the only one on its `from -> to` pair, and every `cap` mention on the cap's predicates sits inside a comparison `quantity OP cap` whose quantity `reads` declares |

...
Every entry in `resume_targets` must be a declared edge out of **one of** that reason's `parks_at` steps, and all 538 of them are — that is what the validator enforces. What defends that set in turn is the other direction: every step where the graph itself parks a reason must appear in that reason's `parks_at`. The check refuses a MISSING producer and says nothing about an extra step, so the failure it closes is not a union computed over steps the flow never stops at — it is a union that leaves out the very step whose guard names the reason, so `resume_targets` is never asked whether it is reachable from where the flow actually stops, and every copy derived from the row repeats the omission. Only that direction is checked — a row may list more, because a reason can also be produced outside the graph, as the daemon's own boundary check does wherever it stops a task. The permission is that union and not the step the flow is at: a reason parking at nine steps permits the edges leaving all nine, so a flow parked at one may resume into a step reachable only from another. Measured on this document, 207 of the 538 are an edge out of EVERY step in their reason's `parks_at` and 331 are not, and a live daemon was observed taking one of the 331. That is the rule, not a gap in it — a narrower one would strip those targets, which is a rewrite of the recovery table rather than a repair, and `the union is deliberate` in the test suite is what makes an attempt to tighten it fail loudly.
...
The resume contract used to be written three times: here as `recovery`, as the park table in the technical plan, and as the resume table in core flows. A fourth, partial copy exists and is not one of the rendered views: `resources/refusal-vocabulary/refusal-vocabulary.v1.json` carries `parks_at` and `parks_at_classes` for each reason and nothing else of the contract, extracted from the park table and checked against it by `validate:refusal-vocabulary`. It is derived rather than hand-kept, which is why it is not a fourth place to drift — but it does move whenever `parks_at` does, and a change that widens a row has to re-extract it. Three hand-kept copies drift, and a check that notices drift makes it detectable rather than impossible. Two of the three are now rendered from this document, and `views[]` is what they are rendered from.
...
Closed set: `graph_cap_transition_unknown`, `graph_digest_stale`, `graph_duplicate_key`, `graph_duplicate_name`, `graph_entry_step_unknown`, `graph_first_step_unknown`, `graph_guard_unknown`, `graph_lone_surrogate`, `graph_not_json`, `graph_number_not_canonical`, `graph_park_reason_ambiguous`, `graph_park_reason_reserved`, `graph_park_reason_unknown`, `graph_predicate_unknown`, `graph_priority_ambiguous`, `graph_recovery_missing`, `graph_recovery_parks_at_incomplete`, `graph_recovery_reason_unknown`, `graph_schema`, `graph_step_unknown`, `graph_step_unreachable`, `graph_terminal_step_leaves`, `no_transition_reason`, `resume_target_not_permitted`, `transition_not_declared`.

...
## ADR-064: недоступный сигнал — не безопасный сигнал, а возраст не является доказательством

- Решение: контракт Housekeeping для issue #30 — `docs/contracts/housekeeping.md`, закрытая схема `resources/housekeeping/housekeeping-report.schema.json`, рабочий отчёт и отвергаемый отчёт, валидатор `scripts/validate-housekeeping.mjs` и 22 теста. Класс `housekeeping_report` добавлен в реестр #14.
- **Существующий `cleanup` удаляет записанные пути — это лёгкая половина.** Тяжёлая — всё, что никто не записывал: worktree, брошенный после падения; orphan-снимок; грязное дерево без текущей задачи; detached или null-ветка; работа, существующая только в submodule; объект, который выглядит старым и удерживается живой блокировкой; каталог, который вообще нельзя классифицировать.
- **Housekeeping — именованная команда, а не скрытый побочный эффект конца Epic.** Удаление — единственная операция, которую нельзя пересмотреть постфактум, поэтому оно не происходит как следствие чего-то другого.
- **Классификация выводится из сигналов с тремя состояниями, и третье — суть.** `unavailable` — не «нет»: отсутствующие метаданные Git, недоступный хост PR и нечитаемый файл блокировки означают «мы не знаем», а незнание не является причиной удалять. Любой недоступный сигнал переводит объект в `unknown`.
- **Возраст никогда не классифицирует.** Брошенный worktree и worktree, удерживаемый трёхнедельной арендой, по возрасту неотличимы, а безопасен для удаления только один из них.
- **Работа в submodule не теряется молча:** чистый суперпроект — ровно то, как выглядит работа, существующая только в submodule. Объект с собственными submodule-ветками или коммитами вне канонической линии — самое большее `review`.
- **Одобряются объекты, а не отчёт.** Одобрение «отчёта» распространилось бы на объекты, появившиеся после его составления, — то самое окно, которое контракт закрывает. Пока пользователь читает отчёт, мир продолжает двигаться, поэтому каждый объект перепроверяется непосредственно перед удалением: задача, стартовавшая между отчётом и одобрением, находит свой worktree на месте.
- **Удаление идёт по одному объекту через доверенный адаптер**, никогда `rm -rf` или `git worktree remove --force` из shell модели; отказ останавливает объект, а не запуск, и результат — три точных списка с причинами. Освобождённый объём измеряется, а не оценивается. Владение доказывается для каждого объекта: «это под нашим корнем» — утверждение о пути, а не о том, кому принадлежит содержимое.
- **Повторный запуск по неизменившемуся хосту не предлагает ничего** — то, что делает его безопасным для частого запуска, и следствие того, что классификация выводится, а не накапливается.
- Мутационная проверка: 32 из 32 guard'ов убиваются тестами.
- Отложено и названо: сборщик инвентаря, реализация классификатора, драйвер удаления и поверхность CLI.
- Источники: issue #30; валидатор `scripts/validate-housekeeping.mjs` и 22 теста.

## ADR-065: Debate отделён от Arena одним вопросом, а медиатор показывает разногласие, а не создаёт согласие

- Решение: контракт Debate для issue #31 — `docs/contracts/debate.md`, закрытая схема `resources/debate/debate-manifest.schema.json`, рабочий манифест и отвергаемый манифест, валидатор `scripts/validate-debate.mjs` и 23 теста. Класс `debate_manifest` добавлен в реестр #14.
- **Arena отвечает на вопросы, которые решает построенный артефакт.** Для вопросов ценностей, приоритетов и компромиссов эквивалента не было, и они уходили в худшие места: в Arena, которая хорошо отвечает на другой вопрос; к одному оркестратору без независимых позиций; в предположения, что и есть способ сделать решение невидимым; либо блокировали проект без компактного пакета, по которому человек мог бы решить.
- **Классификатор — один вопрос: существует ли артефакт, построение которого ответило бы на это?** Если да — это вопрос Arena. Классификатор записывается в манифест, а не остаётся суждением в момент диспетчеризации, потому что соблазн односторонний: Debate дешевле запустить, чем прототип, и он производит нечто, читающееся как ответ.
- **Два гейта, и второй не может предшествовать первому.** Первый спрашивает, тот ли это вопрос и те ли позиции; второй — сколько это будет стоить и кто будет говорить. Ответ на оба сразу означает ответ на второй до того, как решён первый. Оркестратор вправе предложить Debate и не вправе его начать.
- **Ростер — от трёх до пяти различимых позиций** в отдельных сессиях на утверждённых маршрутах: подменённый маршрут — это другой участник. Недоступный или деградировавший участник получает видимую пользователю диспозицию: дебат, тихо продолжившийся с тремя местами из пяти, ответил на вопрос, которого никто не задавал.
- **Первый раунд независим:** первое, что модель делает с видимой позицией, — соглашается с ней. Последующие раунды перекрёстно исследуют названные утверждения, иначе второй раунд — это первый, повторённый увереннее.
- **Медиатор показывает разногласие.** Суммировать легче, когда сводка чистая, а чистая сводка реального разногласия — ложная. Меньшинство и неразрешённые предположения сохраняются: неразрешённое — самая ценная часть артефакта именно потому, что оно не разрешилось.
- **Debate сам по себе ничего не решает:** он не даёт code PASS, не заменяет Panel и review, `accepted` требует записанного решения пользователя, а материальное принятое решение входит в путь пересмотра требований #25, а не применяется напрямую. Раунды и бюджет обеспечивает хост. Перезапуск продолжает тот же ростер: дебат, возобновлённый с другим составом, — новый дебат, унаследовавший авторитет прежних раундов.
- Мутационная проверка: 37 из 37 guard'ов убиваются тестами.
- Отложено и названо: регистрация workflow, runtime участников и медиатора, драйвер раундов и передача пакета решения.
- Источники: issue #31; валидатор `scripts/validate-debate.mjs` и 23 теста.

## ADR-066: walkthrough объясняет и ничего не удостоверяет, а дрейф в объяснении хуже его отсутствия

- Решение: контракт changeset walkthrough для issue #33 — `docs/contracts/walkthrough.md`, закрытая схема `resources/walkthrough/walkthrough.schema.json`, рабочий и отвергаемый примеры, валидатор `scripts/validate-walkthrough.mjs` и 20 тестов. Класс `changeset_walkthrough` (категория `explanatory`) добавлен в реестр #14.
- **После сложной реализации у пользователя есть записи PASS и доказательства, но не обязательно смысловой путеводитель:** что изменилось, почему и в каком порядке это стоит читать.
- **Артефакт объясняющий и никогда не определяющий поведение:** он не создаёт PASS, не заменяет верификацию и доказательства, а отказ от него ни на что не влияет.
- **Предлагается, а не производится.** Объясняющий артефакт, которого никто не просил, не бесплатен: это ещё один документ, способный разойтись с реальностью, а дрейф в объяснении хуже его отсутствия — читатель ему доверяет.
- **Привязан к точной идентичности staging.** Смысловое изменение staging делает walkthrough устаревшим, и устаревший никогда не показывается как текущий: объяснение дерева, которого больше нет, читается ровно так же, как объяснение существующего.
- **Привязка финальной цели не меняет смысловых утверждений:** дайджест по смысловым разделам обязан совпасть, иначе это новый walkthrough, а не привязка. Добавить идентичность цели — бухгалтерия; изменить сказанное об изменении — нет.
- **Порядок чтения основан на риске, а не на алфавите.** Четыре рисковых вида между собой не ранжируются — что читать первым, зависит от изменения, и притворяться иначе значило бы вводить правило ни о чём; зафиксировано то, что механические детали не идут первыми: walkthrough в порядке файлов — это листинг каталога с прозой, тратящий внимание читателя в порядке, который выбрала файловая система.
- **Выполненные и оставшиеся проверки разделены.** Выполненная ссылается на доказательство при точном staged-дереве; оставшаяся не получает доказательства. Два списка отвечают на разные вопросы, и слияние теряет второй — что кому-то ещё предстоит сделать.
- **Факты проверяются детерминированно.** Ценность walkthrough в том, что человек верит ему, не перепроверяя, — поэтому здесь уверенная фраза с неверным идентификатором коммита вредит сильнее всего. При любом расхождении артефакт не публикуется как текущий.
- **Абсолютные пути вынесены в отдельный отказ:** это утечка, переживающая обзор, потому что выглядит как контекст, а не как данные, и несёт имя пользователя.
- Мутационная проверка: 30 из 30 guard'ов убиваются тестами.
- Отложено и названо: генератор, эвристика «существенного изменения» для предложения, обзор качества текста и путь публикации.
- Источники: issue #33; валидатор `scripts/validate-walkthrough.mjs` и 20 тестов.

## ADR-067: детерминированный анализатор — это ворота, а не пятая модель, и он привязан к пяти идентичностям

- Решение: контракт статического анализа для issue #47 — `docs/contracts/static-analysis.md`, две закрытые схемы (`static-analysis-policy`, `static-analysis-result`), пример утверждённой политики, рабочий результат и отвергаемый результат, валидатор `scripts/validate-static-analysis.mjs` и 22 теста. Классы `static_analysis_policy` и `static_analysis_result` добавлены в реестр #14.
- **LLM-агенты производят узнаваемый класс дефектов** — ненадёжные конструкции, security hotspots, дублирование, мёртвый код, сложность, тонкое покрытие нового кода, — который машина находит дешевле и воспроизводимее, чем модельный обзор. Найти их до обзора — значит оставить внимание модели на дефектах, которые может найти только читатель.
- **Анализатор не является пятой моделью и не управляет workflow.** Он дополняет и не заменяет детерминированные тесты и покрытие, четырёхмодельную Panel, независимый cross-family Code Review, aggregate verification, human acceptance и финальный target CAS.
- **Интерфейс provider-neutral по построению:** в схемах нет ни облака, ни плана, ни редакции; SonarQube — первый адаптер, а не интерфейс. Возможность, которой у провайдера нет, **отсутствует, а не аппроксимируется**: режим, который не поддерживается, не даёт PASS.
- **Три состояния и никакого тихого понижения.** `disabled` работает без анализатора и не заявляет static-analysis PASS; `advisory` записывает; `required` блокирует. Недоступные обязательные ворота — это не совещательные ворота, а ворота, которые не отработали, и в этой разнице весь смысл того, что их объявили обязательными.
- **Политика — ровно семь условий `Sonar way for AI Code`,** и pull request применяет четыре из них, а main — все семь. Вычислить не тот набор — не строже и не мягче, а ответ на другой вопрос.
- **Режим малого изменения выключен по умолчанию** либо компенсируется хостом: провайдер, пропускающий покрытие и дублирование ниже порога строк, пропустил бы поток маленьких агентских правок мимо требования, которому ни одна из них не соответствует.
- **Результат привязан к пяти идентичностям:** кандидат (коммит, дерево, pathspec), дайджест политики, идентичность провайдера (сервер, сканер, анализаторы), дайджесты всех входных отчётов и собственная идентичность анализа. Именно так на практике принимается устаревший PASS: дерево изменилось, идентификатор анализа — нет, и зелёная галочка двадцатиминутной давности всё ещё на странице.
- **Происхождение входных отчётов записывается:** без дайджеста и дерева, из которого отчёт получен, «покрытие 84 %» — это число без подлежащего.
- **Диспозиции hotspot и false positive не принадлежат модели-исполнителю.** Причина не в недоверии к конкретному ответу: у исполнителя самая сильная причина считать собственный код безопасным, а это то самое место, где такое убеждение было бы записано как факт о коде.
- **Webhook аутентифицируется, сопоставляется с анализом и проектом, идемпотентен при повторе и игнорируется до записи квитанции операции:** результат для операции, которой хост не записывал, — это результат о том, о чём хост не спрашивал.
- **Отправка кода размещённому анализатору — решение о доставке (#17) и вопрос clearance (#20)**, записываемое до первой отправки. Контракт не создаёт учётной записи, не меняет тариф и ничего не покупает; платная или self-hosted возможность включается только после preflight и утверждённого решения о доставке.
- Мутационная проверка: 43 из 43 guard'ов убиваются тестами.
- Отложено и названо: адаптер, Free-пилот на публичном репозитории, приёмник webhook, офлайн-фейк провайдера для #36 и проверка doctor.
- Классификация: #47 лежит вне матрицы v1 (#3–#39); это кандидат следующей версии матрицы, и до её принятия он не блокирует ни v1, ни релиз (раунд 5 #39, R5-11; debt 9d).
- Источники: issue #47; валидатор `scripts/validate-static-analysis.mjs` и 22 теста.

## ADR-068: неоднозначность снимается на уровне протокола, а не описывается в runbook

- Решение: контракт типизированного write API для issue #38 — `docs/contracts/sdk-write-api.md`, две закрытые схемы (`capability-token`, `write-operation`), пример токена, рабочая операция и отвергаемая операция, валидатор `scripts/validate-sdk-write-api.mjs` и 19 тестов. Класс `sdk_write_operation` добавлен в реестр #14.
- **Дефект не в том, что CLI медленный.** Между намерением и демоном стоят процесс, shell и парсер, и каждый из них — способ ошибиться: экранирование аргументов, версия бинаря из `PATH`, разбор вывода, ориентированного на человека, невозможность атомарно объединить связанные записи. Но главное — падение после коммита демона и до ответа CLI **оставляет исход, который никто не может назвать**.
- **Ревизия и ключ идемпотентности снимают эту неоднозначность на уровне протокола**, а не описывают её в runbook. Потерянный ответ для вызывающей стороны неотличим от отказа, поэтому единственный безопасный повтор — тот, который демон способен опознать: повтор по тому же ключу возвращает тот же исход и не производит второго эффекта.
- **Транзакция применяется целиком или никак.** Частично применённая партия хуже упавшей: следующее решение вызывающей стороны опирается на состояние, не совпадающее ни с одной из веток.
- **Обобщённой мутации JSON нет.** Endpoint произвольного патча — это способ писать поля, для которых никто не писал валидатора, и само его существование делает все прочие гарантии условными: «пока им никто не воспользовался». Метаданные расширения валидируются на стороне демона по версионированной схеме.
- **Токен возможности называет проект, задачу, операцию и точный набор полей**, истекает и может быть отозван; другой проект недостижим по построению. **Модельные процессы токена не держат** — та же граница, что и у отсутствующих mutation-инструментов: процесс, способный чеканить записи, чеканил бы их на всё, что токен разрешает.
- **Машинные коды ошибок, а не проза:** вызывающая сторона, вынужденная сопоставлять текст сообщения, ломается, когда сообщение улучшают. Конфликт дополнительно называет идентичность, с которой он конфликтует, иначе остаётся гадать или повторять вслепую.
- **CLI остаётся — поверх того же API.** Это хорошая операторская поверхность и плохая зависимость для корректности; сохранение его над тем же API не даёт ему разойтись во вторую семантику, а именно это, а не сам CLI, и есть то, что миграция предотвращает.
- Мутационная проверка: 39 из 39 guard'ов убиваются тестами.
- Отложено и названо: сами upstream-примитивы, миграция мест вызова, dual-path тесты паритета и набор fault injection. Issue P2; отдельные примитивы поднимаются до release blocker, если CLI не обеспечивает атомарных гарантий, уже принятых корректностно-критичным путём.
- Источники: issue #38; валидатор `scripts/validate-sdk-write-api.mjs` и 19 тестов.

## ADR-069: авторизация интеграции — запись, а не состояние операции

- Контекст: единственный токен, который пропускает человеческую остановку перед необратимым шагом, был описан прозой в трёх документах и не имел ни контракта, ни закрытой схемы, ни валидатора. Панель первого раунда нашла это независимо в двух местах.
- Решение: `docs/contracts/integration-authorization.md` + закрытая схема + рабочий и отвергаемый примеры + `validate:integration-authorization`. Запись связывает scope и record ID, проект/Epic/run, target ref и его исходный OID, упорядоченный план переходов, финальное дерево, индекс начала остатка и хеш расписки о завершённом префиксе, три головы, под которыми сделана подпись, предыдущую голову цепочки, срок и терминальную диспозицию, а также ID и хеш подписанного `UserDecisionRecord`.
- Обоснование: истёкшая запись не сохраняет ранее выданные разрешения даже посреди плана — после частичного CAS новая запись начинается от **текущего** OID ветки и покрывает только остаток, потому что разрешение двигать `A → B → C` не является разрешением двигать `B → C`, пока это не подписано. Политика проекта такую запись не выдаёт: она покрывает планирование и обратимую непродуктовую область, а движение чужой ветки — ни то, ни другое.
- Следствие: `integration-state/<operation-id>.json` хранит состояние операции и не может подменять авторизацию. Состояние операции — это то, что произошло; авторизация — то, что было позволено, и вывод второго из первого есть способ, которым прерванная интеграция авторизует собственное продолжение.

## ADR-070: материальное решение — это запись в манифесте, а не убедительное предложение в прозе

- Контекст: правило «ровно один блок `autosk-material-decisions`, а несослатая проза не является материальным полномочием» было записано в `01-core-flows.md` §2 и не имело ни схемы, ни проектора, который кто-нибудь мог бы запустить. Панель первого раунда назвала это второй записью без контракта: пока сравнения нет, всякое предложение в черновике — кандидат в ограничение, и Ticket реализует то, что показалось его читателю убедительным.
- Решение: `docs/contracts/material-decisions.md` + закрытая схема + рабочий и отвергаемый примеры + `validate:material-decisions`. Артефакт объявляет свои материальные решения одним машиночитаемым блоком; каждое решение несёт стабильный `decision_id`, `statement`, вид и разделы, которые на нём стоят; каждый поведение-определяющий раздел называет ID, на которых держится.
- Обоснование: два блока отвергаются (`material_manifest_duplicate`), а не сливаются, иначе полномочие артефакта зависит от того, до какого блока читатель дошёл первым. ID стабилен между черновиками: переиспользование его под другое утверждение — это способ, которым одобрение переживает решение, о котором оно было. Раздел, объявленный нормативным и не ссылающийся ни на что, отвергается (`material_section_unmapped`) — не потому что проза запрещена, а потому что раздел, который что-то решает и ни на что не ссылается, и есть то полномочие, которое контракт убирает.
- Следствие: проектор сравнивает и не интерпретирует. Он называет четыре различия четырьмя классами, а не «манифест изменился»: тот, кто чинит выпавшее решение, делает не то же самое, что тот, кто чинит переформулированное. Заморозку разрешают только побайтово эквивалентная проекция или добавления, которые классификатор доказал локальными и нематериальными и назвал реестр, под которым доказывал; «читается так же» побайтовой эквивалентностью не является, и проектор, который бы это принял, интерпретировал бы. Раздел, который был пояснением и остался пояснением, находкой не считается, иначе всякий глоссарий читался бы как регрессия.
- Отложено и названо: сам классификатор локальности — он принадлежит реестру артефактов, и этот контракт потребляет его вердикт, а не переписывает его.
- Мутационная проверка: 19 из 19 guard'ов убиваются тестами.
- Источники: issue #4; панель, раунд 1; валидатор `scripts/validate-material-decisions.mjs` и 13 тестов.

## ADR-071: набор достижимых park-состояний конечен и выводится, а не поддерживается

- Контекст: дизайн говорит, что workflow паркуется в `human` с названной причиной, а не с предложением. Панель первого раунда нашла это утверждение сразу в трёх документах — `01-core-flows.md` §8, `03-technical-plan.md` §7 и контракты — и ни одного места, где набор причин перечислен и проверен. Три места из четырёх сообщили об этом независимо; так снаружи выглядит правило без механизма.
- Решение: `docs/contracts/refusal-vocabulary.md` + закрытая схема + `resources/refusal-vocabulary/refusal-vocabulary.v1.json` + `validate:refusal-vocabulary`. Словарь **извлекается** из resume contract'а `03-technical-plan.md` §7, шаги читаются из зарегистрированных графов workflow §2, и расхождение между таблицей и ресурсом — `refusal_vocabulary_drift`, а не слияние.
- Обоснование: второй список, который может тихо разойтись с первым, и есть тот отказ, который контракт предотвращает, поэтому ресурс никогда не выигрывает спор у таблицы. Колонка шага написана для человека и несёт уточняющую прозу, поэтому из неё берутся только токены, называющие зарегистрированный шаг; опечатка в шаге при этом не проходит молча — строка оказывается не называющей ни одного шага, а причина, которая не паркуется нигде, отвергается. Класс шагов, определённый как «всё остальное», пересчитывается и сравнивается, иначе он превращается в список, который кто-то правит.
- Следствие и честная граница: **71 из 84** причин паркует демон, которого в этом репозитории нет. Каждая запись называет производителя, и заявление опровергается там, где репозиторий с ним не согласен: `host` без файла и `daemon`, который репозиторий на самом деле производит, — два разных отказа. Совпадение по целому слову: короткий код является суффиксом длинных, и файл, который пишет только длинный, короткий не производит. Этот репозиторий проверяет, что причины объявлены, перечислены и привязаны к настоящему шагу; он не проверяет, что демон их испускает, и ни одно число здесь не следует читать как покрывающее это.
- Побочно: `epic-planning-ref.md` и `tickets-manifest.md` не объявляли закрытого набора вообще, хотя §8 и §7 ключуются на их кодах. Теперь объявляют, и `refusal_vocabulary_unclosed_contract` не даёт открыться снова.
- Мутационная проверка: 23 из 23 guard'ов убиваются тестами.
- Источники: issue #4; панель, раунд 1 (opus high 1 и 2, grok medium 1, muse high 4); валидатор `scripts/validate-refusal-vocabulary.mjs` и 13 тестов.

## ADR-072: мутационный прогон — это команда, а не число в pull request'е

- Контекст: пакет панели утверждал, что «каждый runtime-модуль в `src/host/` прошёл мутационную проверку», а счётчики оставлял в pull request'ах, которых в пакете нет и которые не привязаны к digest'у. Три места из четырёх назвали это: самое сильное утверждение о корректности во всём разделе доказательств оказалось непроверяемым из замороженного артефакта.
- Решение: `scripts/mutation-report.mjs` и `npm run mutation-report`. Каждый guard нейтрализуется по одному, свой тестовый файл модуля перезапускается, и отчёт называет модули, мутантов, убитых и выживших. Выживший допустим только там, где кто-то записал причину, **на той строке**, к которой она относится: `resources/mutation-report/mutation-survivors.v1.json`. Выживший, которого никто не назвал, роняет команду.
- Обоснование: смысл не в числе, а в том, что число может пересчитать любой, у кого есть замороженное дерево, — именно это «прошёл мутационную проверку» и должно значить, прежде чем стать доказательством. Пакет называет и сам набор правил мутации, и то, что более широкая ручная процедура, применявшаяся при написании каждого модуля, даёт другое число; смешивать их — значит выдавать одно за другое.
- Что нашёл первый полный прогон: 513 мутантов, 487 убитых, **26 выживших**, и ни один из них не был шумом. Одиннадцать в `scoped-child-creation.mjs`, где цикл отвергающих фикстур утверждал только «что-то бросило»: фикстура «родитель в blocked_by» на самом деле отвергалась совсем другим guard'ом ниже по течению, поэтому guard, по которому тест назван, никогда не вычислялся. Пять в `approved-delta.mjs` — пять отдельных guard'ов над одной записью, ни один не спрошен в одиночку. Два в `daemon-preflight.mjs` и два в `governance-bundle.mjs` — соседние guard'ы с тем же кодом отказа. Два в `evidence-retention.mjs` — фикстура ломала оба поля сразу. Один в `verify-doc.mjs`. И один в `doctor-checks.mjs`, который оказался **мёртвым**: `results.length === registry.length` не может быть ложным, потому что цикл кладёт ровно один результат на запись, — утверждение удалено, свойство стало тестом.
- Следствие: тест, который проверяет только факт броска там, где над одной записью стоит несколько guard'ов, — это утверждение, которое не может провалиться по той причине, ради которой написано. Теперь такие фикстуры называют ожидаемое сообщение.
- Побочно исправлено в пакете панели: закрытые наборы отказов читаются в обеих формах (раунд 1 получил «22 контракта не объявляют закрытого набора», когда их объявляли все); digest'ы файлов приводятся полностью, и названо правило, по которому пересчитывается candidate digest; названы знаменатель матрицы отказов и её перечисление; сказано, что поставляемый источник — upstream **плюс** закреплённая серия патчей, и что именно она даёт три примитива, которых upstream не реализует (неверно: серия даёт только ADR-014, исправлено в ADR-083); идентичность фактически загруженного расширения печатается и сравнивается с рецензируемым деревом — и пакет прямо говорит, если прогон был про другие байты.
- Источники: issue #39; панель, раунд 1 (opus high 4 и 5, grok medium 2, muse high 2 и 3, astra medium 3); `scripts/mutation-report.mjs`, `test/mutation-report.test.mjs`, `test/build-panel-package.test.mjs`.

## ADR-073: реестр артефактов решает, а path-role classifier — описание того, что подразумевалось

- Контекст: один и тот же замороженный кандидат содержал закрытый пятизначный path-role classifier «до issue #14» из `01-core-flows.md` §2 и полный контракт реестра артефактов со своими классами и своим закрытым набором отказов — и ни строки о том, какое из двух правил действует. Панель первого раунда назвала это отсутствием правила перехода.
- Решение: **реестр решает.** Путь, которым управляет класс реестра, получает lifecycle этого класса, и path-role classifier к нему не применяется. Путь, которым не управляет ни один класс, отвергается как `unregistered_artifact` — самый дешёвый lifecycle по умолчанию не выдаётся.
- Обоснование: два правила без порядка — это способ, которым один и тот же файл получает два разных gate'а в зависимости от того, кто его читал. Порядок написан там, где живёт более старое правило, а не только в новом контракте: иначе его прочтёт лишь тот, кто уже знает, что искать.
- Следствие: в поставляемой системе пятизначный classifier недостижим, потому что реестр отвергает неучтённый путь раньше. Он остаётся описанием того, какой lifecycle подразумевался для роли, пока её не внесли одной записью, и исчезает вместе с runtime issue #14. Это честнее, чем удалить его сейчас: пока #14 не реализован, описание ролей — единственное, что говорит, какой класс писать.
- Источники: issue #39; панель, раунд 1 (grok medium 2).

## ADR-074: у каждой park-причины один владелец, и имя, объявленное дважды, отвергается

- Контекст: панель второго раунда независимо у трёх мест нашла, что park-причины четырёх alignment-жизненных циклов — главного человеческого gate'а дизайна — не входят ни в один закрытый набор контракта. Измерение показало картину шире, чем видела панель: **67 из 84** причин не закрыты ни одним контрактом. Отдельно opus заметил, что «378 классов» считает объявления, а не различные имена: `registry_drift` объявляли два контракта и `foreign_ref_movement` — тоже два.
- Решение: у каждой записи словаря появилось поле `closed_by`. Семнадцать причин закрыты контрактом того артефакта, которому принадлежат; остальные шестьдесят семь — собственный словарь workflow и принадлежат resume contract'у `03-technical-plan.md` §7. Запись без владельца или называющая документ, который её не закрывает, — `refusal_vocabulary_owner_missing`.
- Обоснование: «кто-то же где-то это закрыл» — ровно тот способ, которым код без владельца выживает. Владелец должен быть полем, а не выводом читателя; тогда шестьдесят семь причин без контракта перестают быть незамеченным пробелом и становятся записанным фактом, который можно оспорить.
- Имя, объявленное двумя контрактами, отвергается (`refusal_vocabulary_owner_ambiguous`), а не разрешается приоритетом: у него нет ни единственного владельца, ни единственного производителя, ни единственного шага, и вызывающая сторона, ветвящаяся по имени, не может отличить, какое из двух условий получила. Оба столкновения разведены: `foreign_ref_movement` объявляет только `approved-delta.md` (значение одно и то же, кто бы его ни наблюдал), а `execution-base.md` его потребляет; `registry_drift` finding-реестра переименован в `finding_registry_drift` — там это другое условие («неизвестное work state»), а не то же самое.
- Мутационная проверка: 24 из 24.
- Источники: issue #39; панель, раунд 2 (opus high 3 и medium «378», grok high 2, muse medium 1).

## ADR-075: `complete` над выбранным перечислением — это утверждение о перечислении, а не о системе

- Контекст: панель второго раунда у трёх мест независимо сказала одно и то же: матрица из шестнадцати групп не содержала ни одного отказа в окне публикации планирования — ни crash-хвоста между записью объекта и CAS, ни окна передачи live→audit, которое архитектура выделяет отдельно. `covered_by_real_fault=16; complete=true` было полным над выбранным перечислением, пока два инварианта восстановления, на которых стоит вся модель полномочий, оставались непроверенными.
- Решение: граница `planning_publication` и четыре настоящих отказа. F017 — объект коммита записан, ref не двигался: остаётся проверить переживший объект, а не написать второй, потому что новый объект — это другой OID для того же логического коммита. F018 — CAS состоялся, расписка нет: reflog вырос ровно на одну совпадающую запись, поэтому расписка восстанавливается, а ref не двигается второй раз. F019 — keepalive удалён вторым процессом под живой операцией. F020 — окно передачи: audit-копия уже есть, live ещё есть, и это ровно то окно, ради безопасности которого порядок «сначала audit» и выбран.
- Обоснование: у каждого случая свой контроль, и контроль не тот же самый вопрос. Для F017 контроль — состояние без записанного объекта, где «написать» и есть правильное действие; для F019 — целый keepalive из той же фазы, чтобы работу делало слово `invalid`, а не фаза. Guard, отвечающий «проверить» на всё, детектировал бы четыре отказа и не значил бы ничего.
- Следствие: 20 из 20 покрыто настоящим отказом при 20 из 20 молчащих контролях. `complete` по-прежнему означает «полно над этим перечислением» — но перечисление теперь включает границу, которую панель назвала отсутствующей, и пакет называет и знаменатель, и сами группы.
- Источники: issue #36, #39; панель, раунд 2 (opus high 5, grok high 3, muse high 3).

## ADR-076: гарантия без класса — это предложение, и три из них получили механизм

- Контекст: панель второго раунда нашла четыре утверждения, у которых не было того, что бы их вычисляло.
- **Auto-context провайдера.** «Файл, который провайдер загрузил бы сам, в конверт не попадает: auto-context либо отключён, либо целиком перечислен lock'ом» — и ни один из одиннадцати классов `provider-preflight.md`, ни один из десяти классов `project-instructions-lock.md` этого не проверял: первые про маршрут, вторые про сам файл lock'а, а не про то, что провайдер грузит сам. Добавлен `route_auto_context_unpinned`: запись маршрута называет disposition — `disabled` либо `enumerated_by_lock` с точным digest'ом lock'а, который это перечисляет. Маршрут, закреплённый под чужой lock, ничего не говорит о том, что загрузит этот. «Мы не видели, чтобы он что-то грузил» ответом не является, и runner переносит disposition из конфигурации маршрута, а не выводит его из зонда: зонд, ничего не подгрузивший однажды, — не утверждение о настоящем запуске.
- **`governance_mapping_set_digest`.** Он выводился из пятизначного path-role classifier'а, который ADR-073 объявил недостижимым, и при этом входит в идентичность кандидата, а значит и в привязку каждого вердикта. Идентичность не может зависеть от правила, которое другой документ объявляет недостижимым: роль пути для этого набора определяет реестр, а версия — `registry_digest`.
- **Supplementary.** Место семьи автора занимало счётное место в составе из четырёх и имело неназванные полномочия. Теперь названы: оно обязано ответить, как и остальные, но его находки не блокируют в одиночку — Critical или High только от него поднимают вопрос к Lead, который либо подтверждает своим голосом, либо записывает несогласие. Безымянное место с полными правами было бы просто пятым голосом автора.
- **Два написания корня проекта.** Дерево доказательств и protocol snapshot писались под `<absolute-project-root>`, тогда как guard'ы изоляции того же раздела требуют канонический корень. F005 существует ровно потому, что символическая ссылка делает эти два написания разными; теперь написание одно.
- Побочно: `epic_ref_key` в `01-core-flows.md` §2 больше не переизлагается даже частично. Имя ref'а сравнивается побайтово в expected-old CAS, поэтому кодировка входит в определение — она перенесена в единственное определение в `02-architecture.md` §2, а не продублирована с лишним требованием.
- Источники: issue #39; панель, раунд 2 (opus high 4, medium «governance_mapping_set_digest», medium «Supplementary», low «два написания», low «epic_ref_key»).

## ADR-077: у Arena есть контракт, потому что путь, меняющий Tech Plan, не может быть единственным без него

- Контекст: панель второго раунда заметила асимметрию — у Debate есть и контракт, и схема, а у Arena, той ветки, **вместо** которой Debate и выбирают и которая меняет Tech Plan, не было ни того ни другого. `arena_fallback_required` не закрывался ничем.
- Решение: `docs/contracts/arena.md` + закрытая схема блока `autosk-arena` + рабочий и отвергаемый примеры + `validate:arena`. Закрытый набор — девять классов.
- Обоснование: у этой схемы три способа стать театром, и каждый отвергается, а не порицается. **Кандидаты, увидевшие друг друга**, — это один кандидат с лишними шагами, поэтому загрязнение отвергает кандидата, а не понижает его. **Судья, который утверждает, а не ранжирует**, был бы моделью, утверждающей собственное материальное решение, — то самое, что §2 запрещает везде; поэтому решение закрывает только `UserDecisionRecord`, а рекомендация остаётся входом. **Победитель, не вернувшийся в план**: план и есть артефакт под панелью, и план, который никто не переписал, — это план, который никто не прочитал; неизменившаяся идентичность Tech Plan отвергается.
- Рамка закрывается до старта кандидатов: три-шесть измеримых критериев. Меньше трёх — не сравнение; больше шести — обзор, и судья, ранжирующий по дюжине осей, выбирает, какие взвешивать, — решение, которого никто не делегировал. Критерий, написанный после того, как работа появилась, — критерий, написанный под неё, поэтому freeze сравнивается со стартом кандидата, а не принимается на слово.
- Отложено и названо: четыре park-причины жизненного цикла кандидата принадлежат child-workflow в `03-technical-plan.md` §7 и там же остаются. Причина, объявленная двумя документами, не имеет единственного владельца — это то, что отвергает ADR-074.
- Мутационная проверка: 17 из 17.
- Источники: issue #4, #39; панель, раунд 2 (opus low «у Arena нет контракта», grok high 2).

## ADR-078: мутация испускания — не мутация предиката, и число было слабее слова

- Контекст: раунд 3 панели у трёх мест независимо сказал одно: набор из пяти операторов (`demand(`, `errors.push(` и родственные) нейтрализует только **испускание** отказа, до которого уже дошли, и не трогает **предикат**, решающий, сработает ли guard. Значит «514 из 514 убито» доказывало, что тест замечает удалённый push, а не что guard правильно обусловлен. Граница, написанная `>=` там, где нужно `>`, равенство вместо неравенства, `&&` вместо `||` — всё это переживало прогон нетронутым. Заголовочное число было слабее, чем слово «мутация» подразумевает; находка верна и принята целиком.
- Решение: восемь предикатных операторов поверх пяти прежних. Источник сканируется один раз, и мутируются только смещения, которые действительно являются кодом: оператор внутри сообщения, комментария или регулярного выражения — не предикат, и его мутация дала бы либо бессмысленного мутанта, либо синтаксическую ошибку, засчитанную как убийство. Стрелка `=>` исключена явно по той же причине.
- **Что нашлось по дороге, до всякого прогона:** пятнадцать из прежних 514 «убитых» мутантов **были синтаксическими ошибками**. Правило `errors.push(` → `void (` неприменимо к вызову со spread — `errors.push(...someErrors())` — и давало непарсящийся модуль, на котором падал любой тест. Это ровно та нечестность, о которой предупреждала панель, только моя собственная и более ранняя. Правило заменено на `[].push(`, которое вычисляет аргументы, отбрасывает их и парсится в обеих формах; все 1670 мутантов проверены на парсимость до прогона.
- Обоснование: смысл не в том, чтобы число выросло, а в том, чтобы оно значило то, что говорит. Мутант, который не парсится, убивают все тесты сразу и он не проверяет ничего.
- Источники: issue #39; панель, раунд 3 (opus high 1 и medium, muse high 2, grok low 2).

## ADR-079: инъекция и специфичность — разные утверждения, и считаются отдельно

- Контекст: все четыре места раунда 3 указали на одно и то же предложение пакета: «каждая группа внедряется по-настоящему и парована с контролем». Для четырёх групп из двадцати это было неправдой, а сводка всё равно давала `complete=true`. Собственный довод пакета — guard, отвергающий всё, детектировал бы каждый отказ и не значил бы ничего — применим ровно к ним.
- Решение: `control` стал полем строки покрытия, а `controlled` — отдельным счётчиком рядом с `complete`. Пакет говорит прямо: `complete` означает, что каждая группа внедрена, а не что каждый guard показан специфичным.
- Уточнение, которое дала astra и не дали остальные: **F004 контроль имеет** — identity-harness выполняет и настоящий resume под подменёнными байтами, и «те же байты возобновляют задачу» как контроль. Просто строка доказательства этого не называла: она описывала удержание пина и следующее допущение, но не resume, которым группа определена. Строка исправлена, поле проставлено. Без контроля остаются три группы crash-harness'а, который внедряет в двух точках записи и не задаёт вопрос о ненарушенном состоянии, — и теперь это сказано, а не оставлено читателю на вывод.
- Источники: issue #36, #39; панель, раунд 3 (opus high 3, astra medium 1 и 2, grok medium 2, muse high 3).

## ADR-080: cross-family независимость — записанный раздел, а не соглашение об именовании

- Контекст: раунд 3 (opus low) заметил, что вся панельная гарантия, выбор Lead и `arena_judge_family_conflict` стоят на понятии «семья», которое утверждается формой строки маршрута и не закреплено ни одним артефактом. §6 при этом прямо говорит, что Codex — та же семья, что GPT, но проверить это было нечем.
- Решение: `resources/panel-roster/family-partition.v1.json` называет семьи, их префиксы маршрутов и мастер-порядок; `validate:provider-preflight` отвергает раздел, где семья объявлена дважды, где мастер-порядок ранжирует не те семьи, что объявлены, где обязательный маршрут не принадлежит ни одной, и где четыре обязательных маршрута не покрывают четыре **разные** семьи.
- Обоснование: четыре места в трёх семьях — это трёхмодельная панель под четырьмя именами, и отличить её от настоящей без записанного раздела было нельзя. Проверка привязана к тем же четырём маршрутам, которые уже требует preflight, поэтому подмена маршрута и подмена семьи ловятся одним и тем же местом.
- Источники: issue #26, #39; панель, раунд 3 (opus low «family partition»).
- Дополнено 2026-09-21: ключ раздела — модель, а не префикс маршрута. `cursor/` обслуживает и grok, и kimi (Muse ходит как `cursor/muse-spark-1.3`), поэтому `route_prefixes` заменены списком `models`; модель, которой нет ни в одном списке, отказывает по имени, а не наследует семейство префикса.

## ADR-081: 117 выживших предикатных мутантов — 99 пробелов в тестах, 2 мёртвых условия и 1 настоящая дыра

- Контекст: ADR-078 добавил предикатные операторы, и «514 из 514 убито» превратилось в **1670 мутантов, 1553 убито, 117 выживших**. Каждый выживший — это условие, которое могло быть неверным, и ни один тест бы не заметил. Разобраны все 117.
- **99 оказались настоящими пробелами в тестах**, и три стоит назвать. Условие проверки `security.signer_boundary` — той, от которой зависит, запустится ли модель, — было проверено только на счастливом пути: `reachable && distinct` могло быть `reachable || distinct`, и всё осталось бы зелёным. Три места панели за два раунда называли эту границу подозрительной; мутация подтвердила механически. В `daemon-preflight.mjs` **ни одна граница длины не проверялась на самой границе** — ни имя из 128 символов, ни список из 64 возможностей, ни метод нулевой длины: граница, которую никто не проверяет на границе, — это число в комментарии. В `write-driver.mjs` каталог, стоящий на месте артефакта, читался бы как «там ничего нет» — единственный ответ, который позволил бы записи состояться.
- **2 оказались мёртвыми или избыточными условиями и удалены.** `answered.length > 0 && carriers.size === 1` в `panel.mjs`: множество строится из ответивших мест, поэтому «ровно один носитель» уже означает «кто-то ответил». Условие, которое не может изменить ответ, читается как гарантия и ничего не гарантирует.
- **1 привёл к настоящей дыре.** В `project-fs.mjs` помощник `parentOf` расходился с мутантом только при корне проекта `/` — и там оригинал **допускал** запись. Все проверки вложенности читаются как «внутри `<root>/`», а при `<root>` равном `/` это читается как «внутри чего угодно»: единственный ответ, которого адаптер не должен давать никогда. Корень файловой системы теперь отвергается при построении адаптера, и только после этого мутант стал эквивалентным — по доказательству, а не по допущению.
- **18 названы эквивалентными**, каждый с доказательством недостижимости: компараторы сортировки, чья ветвь ничьей требует дублирующегося ключа, который отвергается раньше (`tickets_duplicate_id`, `containment_mismatch`, `ambiguous_class`, уникальность путей в дереве Git); `dirname`-помощники, чей нулевой случай недостижим с любого места вызова; и `reported_as`, который читается только там, где обе версии дают не то значение, которое guard отвергает.
- Побочно исправлены два дефекта самого harness'а. **Пятнадцать из прежних 514 «убийств» были синтаксическими ошибками**: правило `errors.push(` → `void (` неприменимо к вызову со spread. Мутант, который не парсится, убивают все тесты сразу и он не проверяет ничего; правило заменено на `[].push(`, и все 1670 мутантов проверены на парсимость **до** прогона. И **убитый прогон оставлял мутированный модуль в рабочем дереве** — `finally` при SIGKILL не выполняется, а следующий `git add -A` закоммитил бы нейтрализованный guard. Оригинал теперь пишется рядом до мутации, и первое, что делает следующий прогон, — восстанавливает оставшееся; проверено на настоящей остановке, а не на смоделированной.
- **Третий дефект harness'а нашёлся через исчерпание памяти машины.** Зациклившийся предикатный мутант оставлял осиротевший тестовый процесс: таймаут убивает прямого потомка, но `--test-isolation=process` порождает внука, который переусыновляется к init и продолжает жечь ядро. Один такой мутант держал 99% CPU одиннадцать минут. Harness, который течёт процессом на каждый зависший мутант, не может завершить прогон, в котором такие мутанты есть; переход на `--test-isolation=none` убрал внуков и заодно ускорил прогон вдвое.
- Итог: **1671 мутант, 1653 убито, 18 названо**. Число теперь значит то, что говорит.
- Источники: issue #39; панель, раунд 3 (opus high 1, muse high 2, grok low 2).

## ADR-082: граф workflow становится документом, и условие перехода — это guard, а не второй механизм

- Контекст: критерий 2 issue #10 требует, чтобы `workflow_graph_digest` покрывал шаги, переходы, guards, потолки и recovery-цели. Патч `0005` строит дайджест из зарегистрированных шагов и имён их хуков и **прямо отказывается** от четырёх составляющих из пяти; переходы живут в потоке управления TypeScript, а 95 рёбер написаны прозой, которую скрапер выбрасывает. Риск 4 в разделе ниже описывает ровно это. Срез 1 закрывает не риск целиком, а его основание: контракт документа, его каноническую сериализацию и её эталон.
- **Guard — это и есть условие, второго механизма нет.** `guards[]` объявлены на верхнем уровне и адресуются по идентификатору; переход называет привязанные к нему guards, и держать должна их конъюнкция. Два альтернативных пути к одному шагу — это два ребра, а не дизъюнкция внутри одного. Обоснование: только так «первый ложный guard в каноническом порядке идентификаторов» — определённый отказ, а не описание порядка вычисления какой-то одной реализации. Предикаты при этом перечислены, а не выражены: язык выражений потребовал бы собственной спецификации, парсера и мутационного покрытия, а цена ошибки в нём — молча разрешённый переход.
- **Две записи одного целого и две записи одной строки сходятся, а дублирующийся ключ отвергается.** Каноническая сериализация на то и каноническая, что отображает много записей в одну: `1` и `1e0` дают одни байты, `s` и `s` дают одни байты. Информацию теряет только дубликат ключа — разобранный объект уже схлопнул его, и схема работает поверх того, где этой информации нет. Поэтому отказ происходит в **разборе**, и тест показывает, что схема дубликат не видит.
- **Эталон канонизации привязан к дайджесту поставляемого примера.** Вход эталона — тот же рабочий пример, переписанный с переставленными ключами, развёрнутыми неупорядоченными массивами и другим отступом. Он воспроизводит `51c273e1…` — дайджест, который пример несёт. Эталон, доказывающий собственный дайджест, доказывал бы только сам себя.
- **Три причины уровня графа закрыты контрактом, а не внесены в словарь отказов.** `resources/refusal-vocabulary/refusal-vocabulary.v1.json` — это перечисление park-состояний **воркфлоу autosk**, извлекаемое из таблицы §7 `03-technical-plan.md`, и `driftErrors` двусторонний: код в ресурсе, которого нет в таблице, — это дрейф. Внести `transition_not_declared`, `resume_target_not_permitted` и `no_transition_reason` туда значило бы дописать строки в документ класса `project_planning_doc` и утверждать, что это достижимые состояния сегодняшнего воркфлоу. Это неправда: они принадлежат runtime графа, которого ещё нет. Контракт владеет ими и закрывает их у себя — ровно так же, как `docs/contracts/execution-base.md` владеет своим набором, которого в словаре тоже нет.
- **Схема написана в положительном подмножестве, которое реализует общий проверщик.** `validateJsonSchema` не поддерживает `not` и `anyOf`; вместо отрицания запреты выражены суженным набором свойств с `additionalProperties: false` в ветке. Альтернатива — дописать два ключевых слова в движок, который читают два десятка валидаторов, — это радиус поражения больше самой задачи, а суженный набор свойств не костыль, а обычный JSON Schema.
- **Срез поставляет `workflow-graph.example.json`, а не `workflow-graph.v1.json`,** как говорит буква плана. Решение владельца: пример остаётся примером, настоящий документ создаёт срез 2. Иначе срез 1 доставил бы заглушку под именем настоящего артефакта. Отклонение принято явно, а не молча.
- **Кросс-семейное ревью (astra, low) нашло четыре дефекта при 1882 зелёных тестах, и три из них — дыры в закрытости.** `__proto__` присваивался как обычный ключ, то есть двигал прототип вместо создания собственного свойства: закрытая схема читает собственные свойства, поэтому такой ключ был ей невидим, а сериализатор его молча выбрасывал — документ говорил то, чего дайджест не покрывал. Целочисленность проверялась **после** превращения токена в `Number`, поэтому `1.00000000000000001` становился ровно единицей, а `1e-4000` — нулём: обе дроби принимались как целые, и эталон переставал доказывать согласие двух реализаций, потому что реализация с точной десятичной арифметикой отвергла бы обе. Описание `park_reason` обещало, что код вне словаря отвергается валидатором, а проверялось только написание — произвольный код с правильной формой проходил, и значения `graph_reasons` можно было переименовать. Четвёртый: `graph_schema` был достижимым, но не объявленным классом, а тест «контракт закрывает всё, что валидатор производит» перебирал руками написанный список и потому расхождения увидеть не мог.
- Следствие для конструкции, а не только для трёх мест: **перечень, сверяемый сам с собой, не закрыт**. Теперь батарея испорченных документов производит отказы, и коды сверяются с объявленным набором — список, отставший от кода, роняет набор тестов. Целочисленность читается по написанным цифрам, схождение `1`, `1.0`, `1e0` и `10e-1` сохранено. Причины отказа сверяются со словарём воркфлоу плюс тремя, которыми владеет контракт; `graph_reasons` закреплены через `const`.
- **Узкое переревью нашло два дефекта, внесённых самими исправлениями, и оба — про цену, а не про логику.** Читая целочисленность по написанным цифрам, я сдвигал десятичную точку дополнением строки — то есть выделял память пропорционально **значению** экспоненты, а не длине токена: `1e1000000000` это двенадцать символов и запрос на миллиард. `Number` этого не делал никогда, так что это регрессия правки, а не непокрытая дыра. Заодно `0e1000000000` — законная запись нуля — стал отвергаться падением. Теперь ноль решается до касания экспоненты, число значащих цифр **считается**, а не строится, и всё длиннее шестнадцати отвергается до любого выделения. Второй: авторитетный набор причин лежал в одном module-global `Set`, который отдавался наружу — вызвавший мог его расширить и тем поменять то, что примут все последующие проверки, а второй root молча получал словарь первого. Кэша нет, каждый вызов читает файл и отдаёт свежий набор.
- Общее у обоих раундов ревью: **зелёный набор тестов не увидел ни одного из шести дефектов**. Первые четыре — при 1882 зелёных, вторые два — при 1892. Это довод не за «больше тестов», а за то, что кросс-семейная проверка ловит класс дефектов, недоступный автору: три из шести — про то, что гарантия имеет неверную *форму* (перечень, сверяемый сам с собой; набор, который вызывающий может расширить; цена, зависящая от значения входа).
- **Раунд 2 нашёл седьмой дефект, и он о различии, которое я стёр: владеть кодом и иметь право его назвать — разные вопросы.** Проверяя причины отказа, я объединил словарь воркфлоу с тремя кодами, которыми владеет контракт, и применил объединение ко всем полям. В результате обычный guard мог нести `no_transition_reason` — код, про который §9 говорит, что ни один валидный документ его не производит, — а обычный шаг мог нести `transition_not_declared`, то есть объявить ребро, отвергнутое по причине, которая существует ровно тогда, когда ребра нет. Теперь разрешённое зависит от места: шаг, guard, потолок и recovery-строка называют только словарь воркфлоу, три зарезервированных отвергаются с `graph_park_reason_reserved`, а в `graph_reasons` они закреплены через `const`. Владение кодами не изменилось.
- Итог по семи дефектам: **ни один не был найден набором тестов** — 1882, потом 1892, потом 1898 зелёных. Четыре из семи — про форму гарантии, а не про её содержание: перечень, сверяемый сам с собой; набор, который вызывающий может расширить; цена, зависящая от значения входа; и разрешение, не зависящее от места употребления. Это довод за кросс-семейную проверку как за механизм, а не как за формальность: автор проверяет то, что имел в виду, и потому не видит именно тех мест, где имел в виду не то.
- Источники: issue #10, #39, #43; технический план, разделы D1, D3a, D4, D5, D6; панель по плану, раунды 1 и 2; кросс-семейное ревью среза 1: раунд 1 (astra low, ASTRA-S1-01…04), узкое переревью (S1-05, S1-06), раунд 2 полным проходом (ASTRA-R2-01).

## ADR-083: серия поставляет ADR-014, а не три примитива, и preflight отказывает, пока два из них не закреплены

- Контекст: раунд 5 панели #39 (R5-1…R5-5) показал, что `02-architecture.md` §3 и пакет панели утверждали, будто закреплённая серия поставляет все три обязательные поверхности: ADR-014, ADR-023 и ADR-025. Это неверно. Ни `UserDecisionRecord` journal и protected heads, ни `authorityGuard`/`integrateApproved`, ни step-capability metadata CAS, `orchestrateChildBatch` и gate-result receipts нет ни в одном из 49 патчей, ни в одном модуле этого репозитория. Custody helper, сопоставленный с `0016`–`0022` и `0024`, на деле store-lock helper, его протокол и исправления доверенной записи состояния (`0019` — файлы настроек и токена); ни один патч серии не трогает `refs/autosk` или `src/git`, а ref-custody helper из §2 (`src/git/ref-custody-helper.ts`) не существует. Та же фраза, записанная в ADR-072 как побочное исправление пакета после раунда 1 («именно она даёт три примитива»), была ошибкой, а не исправлением.
- **Решение: поставленное и обязательное названы раздельно.** §3 перечисляет то, что серия действительно поставляет (ADR-014 и `task.creation-binding` v2, snapshot store и admission, подпись creation grant, store-lock helper), и отдельно — три поверхности, которых нет: ADR-023, ADR-025 и ref-custody helper. Это обязательства фазы реализации #40, а не поставленные возможности. Пакет панели говорит то же в §1 и называет их в §5.
- **Решение: preflight называет все три и отказывает, пока два не закреплены.** `REQUIRED_DAEMON_CAPABILITIES` состоит из закреплённого `task.creation-binding` v2 и двух незакреплённых примитивов: `authority.user-decision` (ADR-023) и `workflow.custody` (ADR-025). Незакреплённый — значит без ревизии и без методов. Отчёт демона, который называет такое имя, — утверждение, которое нечем проверить, поэтому оно не удовлетворяет требование. Требование не параметризуется: `requireDaemonCapabilities(report)` сверяет только с этим набором, а чтение и закрепление отчёта вынесено в `readCapabilityReport`, которое ничего не допускает. Параметр, заменявший набор, позволял передать закреплённый набор и допустить демон без ADR-023 и ADR-025 — это нашло независимое ревью (R9a-1). Расхождение ревизии или методов у закреплённой capability называется раньше отсутствия незакреплённых, иначе оно навсегда пряталось бы за `missing`. Правило «ни одного model workflow без всех трёх» становится свойством механизма, а не только текста: сегодня preflight отказывает демону, собранному из серии, с `daemon_capability_missing`, где `missing` — эти два имени.
- Альтернатива 1: закрепить для ADR-023 и ADR-025 методы по аналогии (`decision.append`, `task.metadata_cas`). Отвергнута: дизайн не называет RPC-поверхность этих примитивов, и выдуманные имена превратили бы требование в описание несуществующего интерфейса. Имя без методов честнее: оно говорит, что требуется, и не притворяется, что известно, как это проверить.
- Альтернатива 2: исправить только текст и оставить preflight как есть. Отвергнута: тогда правило «preflight запрещает любой model workflow» было бы ложным ровно для двух примитивов из трёх.
- Когда примитив будет специфицирован и реализован, он переходит в закреплённый набор с ревизией и методами. Тест, сверяющий требования с исходником поставляемого демона, падает, если демон начинает объявлять незакреплённое имя: объявление без закрепления здесь было бы тихим расхождением.
- Что не сделано и названо: preflight пока не вызывается ни одним путём запуска workflow. Вызов при загрузке расширения (`docs/contracts/creation-grant.md` §5), то есть до любого model launch и child create, — обязательство реализации, как и сами примитивы.
- Источники: issue #39, раунд 5 (`resources/design-candidate/panel/round-5.json`, R5-1…R5-5); ADR-001, ADR-014, ADR-023, ADR-025; `02-architecture.md` §2–§3.

...
Любой verified candidate, который оставляется без публикации, сначала проходит одну helper-транзакцию live-to-audit с `reason=superseded`: это обязательно перед `rebuild_anchor`, повторным `record_artifact_pass`, возвратом в `fix_artifact` или mint new candidate в `freeze_artifact`. Транзакция создаёт exact audit ref, удаляет live ref по expected-old, записывает receipt и append-only terminal record в `planning.candidate_history`; только после read-back разрешён новый candidate. Повторный вход использует тот же operation ID и не создаёт второй ref.
...
| freeze_artifact | candidate_keepalive_op absent and frozen snapshot commit/tree/candidate identity exact | persist closed candidate_keepalive_op phase=prepared before ref side effect; freeze_artifact |
| freeze_artifact | candidate_keepalive_op phase=prepared and keepalive ref absent | sanitized missing-old CAS creates refs/autosk/epics/<epic_ref_key>/candidates/<candidate_identity> at snapshot commit with exact message; phase=ref_created; freeze_artifact |
| freeze_artifact | candidate_keepalive_op phase=prepared or phase=ref_created, keepalive ref points to snapshot commit with matching create reflog proof | reconstruct create receipt, verify complete commit/tree/blob closure and atomically phase=verified; freeze_artifact |
...
| cleanup | enumerate `refs/autosk/epics/<epic_ref_key>/candidates/*`; any live ref lacks exactly one matching current/history candidate_keepalive operation, or unresolved current keepalive candidate_keepalive phase=prepared\|ref_created\|verified exists | human с park.reason=planning_candidate_keepalive_invalid; cleanup cannot delete or ignore a live closure ref; resume cleanup after exact custody repair |
| cleanup | все sandboxes/snapshots удалены либо absent, live candidate namespace empty, all planning.candidate_history records phase=released\|audit_retained with exact audit refs/receipts, dirty=false | done |

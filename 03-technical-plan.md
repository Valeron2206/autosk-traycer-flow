...
| publish_planning_invalidation | phase=ref_advanced and current bindings drifted but recorded operation bytes/ref/reflog exact | atomically phase=verified/effective_target_step=prepare_anchor_impact, update historical planning head/tree/generation, anchor_rebuild_op phase=closed_with_pending_anchor, archive operations and ensure pending_anchor; prepare_anchor_impact |
| publish_planning_invalidation | phase=ref_advanced and current bindings exact | atomically phase=verified/effective_target_step=payload.recorded_target_step, update planning head/tree/generation, anchor_rebuild_op phase=closed with closed commit/generation, archive operations; transition exactly to stored clarify_alignment or present_tickets_breakdown target |
...
- Planned implementation запрещён до Published PASS каждого реально созданного артефакта: disposition=pass с verdict либо disposition=waived с signed exact daemon waiver, publication_status=verified, matching planning_publication_op phase=verified, published_commit_oid и published tree совпадают с live private planning ref, а current publication bindings совпадают с current anchor/protocol/runtime/instruction/artifact/alignment identity.
...
- Tickets не исполняются без current Tickets Published PASS: disposition=pass|waived, publication_status=verified, matching planning_publication_op phase=verified, published_commit_oid и published tree совпадают с live private planning ref, а current publication bindings совпадают с current anchor/protocol/runtime/instruction/artifact/alignment identity; waived требует signed full-skip authority current identity.

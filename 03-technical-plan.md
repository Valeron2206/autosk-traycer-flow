...
| tickets_manifest_invalid | validate_tickets_manifest before freeze; dispatch_ticket_dag after publication | corrected canonical manifest and regenerated views pass Schema, semantic, DAG, indexed-overlap, lineage and exact-byte validation; any set/subject change returns through present_tickets_breakdown and fresh alignment; Panel/child/blocker side effects remain absent until validation succeeds |
| tickets_manifest_stale | validate_tickets_manifest before freeze; dispatch_ticket_dag after publication | current planning/alignment/runtime/protocol/instruction/publication bindings are restored or the stale Tickets candidate/receipt/PASS is voided through a new artifact cycle; a final receipt is rebound to the exact frozen tree before Panel or dispatch |

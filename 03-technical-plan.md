...
| init_planning_ref / freeze_artifact / rebuild_anchor / publish_artifact_pass / publish_planning_invalidation | ref custody ownership/mode/generation drift, helper identity mismatch or unauthorized refs/autosk write probe succeeds | retain every candidate_keepalive; human с park.reason=planning_ref_capability_missing; model/ref/cleanup side effects absent |
...
| publish_artifact_pass | phase=prepared or phase=commit_created and current binding drift before ref movement, ref=expected parent and reflog prefix=checkpoint | atomically phase=voided_before_ref, terminal_reason=binding_drift, recovery/effective target=prepare_anchor_impact, artifact_pass keeps disposition/identity but publication_status=voided_before_ref with terminal_reason/publication_operation_id/supersession binding, archive operation in publication_history and ensure pending_anchor; prepare_anchor_impact |
...
| publish_artifact_pass | phase=verified, candidate_keepalive phase=verified, planning ref/commit/tree, recorded reflog tail, ref custody generation/policy and complete candidate closure exact | under the exclusive ref-custody helper lock verify the exact recorded planning tail, then one atomic update-ref transaction verifies planning ref=expected commit and deletes candidate_keepalive with snapshot commit expected-old; record custody/tail-bound transaction/release receipt or reconstruct observed-absent release only from the same proofs, atomically candidate_keepalive phase=released; publish_artifact_pass |
...
| publish_artifact_pass | phase=voided_before_ref and recovery_target_step=prepare_anchor_impact | idempotently retain original disposition and publication_status=voided_before_ref with terminal bindings; operation is terminal and cannot move ref; prepare_anchor_impact |
...
| publish_planning_invalidation | pre-CAS impact/anchor/projection drift, ref=expected parent and reflog prefix=checkpoint | atomically planning_publication_op phase=voided_before_ref/effective_target_step=prepare_anchor_impact, anchor_rebuild_op phase=voided_before_ref with terminal reason/publication operation ID, preserve pending_anchor and archive both operations; prepare_anchor_impact; no ref movement |
...
| publish_planning_invalidation | phase=ref_advanced and current bindings drifted but recorded operation bytes/ref/reflog exact | atomically phase=verified/effective_target_step=prepare_anchor_impact, update historical planning head/tree/generation and last_verified_reflog_tail, anchor_rebuild_op phase=closed_with_pending_anchor, archive operations and ensure pending_anchor; publish_planning_invalidation |
| publish_planning_invalidation | phase=ref_advanced and current bindings exact | atomically phase=verified/effective_target_step=payload.recorded_target_step, update planning head/tree/generation and last_verified_reflog_tail, anchor_rebuild_op phase=closed with closed commit/generation, archive operations; publish_planning_invalidation |
| publish_planning_invalidation | phase=verified, candidate_keepalive phase=verified, planning ref, recorded reflog tail, ref custody generation/policy and complete invalidation closure exact | under the exclusive ref-custody helper lock verify the exact recorded planning tail, then one atomic update-ref transaction verifies planning ref=expected commit and deletes candidate_keepalive with snapshot commit expected-old; record/reconstruct custody/tail-bound release receipt and atomically candidate_keepalive phase=released; publish_planning_invalidation |
| publish_planning_invalidation | phase=verified, candidate_keepalive phase=released and effective_target_step is stored | read-back exact commit/ref/reflog/release receipt and complete only missing closure/history projection; transition to stored effective target |
| publish_planning_invalidation | phase=voided_before_ref and effective_target_step=prepare_anchor_impact | idempotently retain archived terminal publication/rebuild operations and pending_anchor; prepare_anchor_impact; never move ref |
...
| cleanup | unreleased candidate_keepalive exists | human с park.reason=planning_candidate_keepalive_invalid; cleanup cannot delete a live closure ref |
| cleanup | все sandboxes/snapshots удалены либо absent, all candidate_keepalive records released or retained by audit policy, dirty=false | done |

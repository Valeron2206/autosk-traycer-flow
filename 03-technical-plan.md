...
- privileged install creates a service-owned bare custody Git directory outside project `.git`, pins the non-writable ancestor/device/inode chain, keeps protected refs loose with `gc.packRefs=false`, owns custody packed-refs, and rejects any protected packed entry before model launch;

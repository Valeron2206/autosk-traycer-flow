...
Любой verified candidate, который оставляется без публикации, сначала проходит закрытый `candidate_supersession_op`; общей фразы без durable operation недостаточно. Operation записывается до helper call как `{schema,operation_id,source_step,source_candidate_identity,replacement_intent_digest,keepalive_operation_id,live_ref,audit_ref,snapshot_commit_oid,helper_request_binding,phase:prepared,audit_receipt:null,helper_transaction_receipt:null,created_at_utc,archived_at_utc:null}`. `helper_request_binding` заранее связывает supersession operation/intent, keepalive operation, exact helper request ID, nonce и signed body hash отдельным domain-separated digest; recovery всегда ищет helper journal по этому binding, а не выбирает между двумя operation IDs. `source_step` закрыт значениями `rebuild_anchor|record_artifact_pass|fix_artifact|freeze_artifact`, а phases — `prepared -> audit_transferred -> archived`. Одинаковая candidate identity означает retry и не создаёт supersession; отличающаяся replacement identity/intent требует завершить operation до mint. Helper-транзакция с `reason=superseded` создаёт exact audit ref и удаляет live ref по expected-old; если exact audit ref уже существует, она verifies audit and deletes only the live ref. Lost response восстанавливается только по persisted helper request binding, exact live-absent/audit-present observation and receipt. `audit_transferred` append'ится в `planning.candidate_history` до очистки current candidate; только archived разрешает целевой шаг и новый candidate. Cleanup сверяет как live, так и audit namespaces с current operation/history.
...
| rebuild_anchor / record_artifact_pass / fix_artifact / freeze_artifact | candidate_supersession_op phase=prepared, live ref exact and audit ref absent | helper retain_audit transaction uses same operation ID and reason=superseded; record/reconstruct exact audit receipt and atomically phase=audit_transferred; repeat same source step |
| rebuild_anchor / record_artifact_pass / fix_artifact / freeze_artifact | candidate_supersession_op phase=prepared, live ref exact and exact audit ref already retains the same snapshot | helper retain_audit verifies the existing audit ref and deletes only the live ref; bind the same persisted helper request and atomically phase=audit_transferred; never overwrite audit custody; repeat same source step |
...
| freeze_artifact | candidate_keepalive_op absent and candidate tree/base exact | compute snapshot_commit_recipe and expected OID without writing Git, derive candidate identity/ref, atomically persist closed candidate_keepalive_op phase=prepared with operation ID and null object/ref receipts; freeze_artifact |
...
| freeze_artifact | signed panel waiver mode=full_skip exact current artifact/alignment identity and candidate_keepalive phase=verified валиден | record_artifact_pass с disposition=waived; panel child не создаётся |
| freeze_artifact | candidate_keepalive phase=verified and current_cycle.full_panel_required=true или current_cycle.narrow=false | dispatch_panel |
...
| synthesize_panel | подтверждены findings and candidate_keepalive phase=verified | atomic helper transaction creates audit candidate ref at snapshot commit and deletes live candidate ref, record audit receipt, append terminal record to planning.candidate_history and phase=audit_retained; synthesize_panel |
...
| narrow_review_join | Lead status=done, NOT_PASS/findings and candidate_keepalive phase=verified | atomic helper transaction creates audit candidate ref at snapshot commit and deletes live candidate ref, record audit receipt, append terminal record to planning.candidate_history and phase=audit_retained; narrow_review_join |
...
| publish_artifact_pass | phase=verified, candidate_keepalive phase=verified, planning ref/commit/tree, recorded reflog tail, ref custody generation/policy and complete candidate closure exact | under the exclusive ref-custody helper lock verify the exact recorded planning tail, then one atomic update-ref transaction verifies planning ref=expected commit, creates the audit candidate ref at snapshot commit and deletes candidate_keepalive with snapshot commit expected-old; record custody/tail/audit-bound transaction/release receipt or reconstruct only from the same proofs, atomically candidate_keepalive phase=released; publish_artifact_pass |
| publish_artifact_pass | phase=verified, candidate_keepalive phase=verified and exact audit candidate ref already exists from a prior identical candidate | under the helper lock verify planning ref/tail and audit ref=snapshot OID, delete only the live keepalive by expected-old in the same transaction, bind the same release receipt fields and atomically phase=released; never overwrite the retained audit ref; publish_artifact_pass |
| publish_artifact_pass | phase=voided_before_ref, candidate_keepalive_op phase=verified and recovery_target_step=prepare_anchor_impact | atomic helper transaction creates audit candidate ref and deletes live candidate ref, record audit receipt and phase=audit_retained; publish_artifact_pass |
...
| rebuild_anchor | affected planning kinds не пусты, current published projections and invalidation candidate_keepalive phase=verified exist | bump anchor, re-bind unchanged unaffected passes/alignments only by approved impact; void affected bindings; set current kind=earliest affected and current_cycle.full_panel_required=true; atomically prepare typed descendant planning_publication_op payload.kind=anchor_invalidation binding the keepalive, store clarify_alignment\|present_tickets_breakdown only in post_publication_target_step/payload.recorded_target_step, and set anchor_rebuild_op phase=ready_to_transit. rebuild_anchor records immediate target=publish_planning_invalidation; publish_planning_invalidation |
...
| publish_planning_invalidation | phase=verified, candidate_keepalive phase=verified, planning ref, recorded reflog tail, ref custody generation/policy and complete invalidation closure exact | under the exclusive ref-custody helper lock verify the exact recorded planning tail, then one atomic update-ref transaction verifies planning ref=expected commit, creates audit candidate ref at snapshot commit and deletes candidate_keepalive with snapshot commit expected-old; record/reconstruct custody/tail/audit-bound release receipt and atomically candidate_keepalive phase=released; publish_planning_invalidation |
| publish_planning_invalidation | phase=verified, candidate_keepalive phase=verified and exact audit candidate ref already exists from a prior identical invalidation candidate | under the helper lock verify planning ref/tail and audit ref=snapshot OID, delete only live keepalive by expected-old, record/reconstruct the release receipt and atomically phase=released; publish_planning_invalidation |
| publish_planning_invalidation | phase=voided_before_ref, candidate_keepalive_op phase=verified and effective_target_step=prepare_anchor_impact | atomic helper transaction creates audit candidate ref and deletes live candidate ref, record audit receipt and phase=audit_retained; publish_planning_invalidation |
...
| cleanup | audit_candidate_housekeeping_op phase=prepared and exact audit ref/OID still present | helper delete_expired_audit uses the same operation ID and expected-old; record/reconstruct exact helper evidence and atomically phase=ref_deleted; cleanup |
...
`planning_ref_init_op` имеет phases `prepared -> ref_created -> verified` и требует operation-specific reflog proof before adopting a ref already at base. `candidate_keepalive_op` использует те же phases, missing-old CAS и create receipt, но указывает на frozen snapshot commit и до release удерживает complete commit/tree/blob closure. `planning_publication_op` связывает exact verified keepalive, typed payload=`artifact_pass|anchor_invalidation`, full persisted/read-back exact commit recipe including `commit_object_bytes_base64`, phases `prepared -> commit_created -> ref_advanced -> verified`, terminal `voided_before_ref`, exact expected parent/tree/commit, `reflog_checkpoint` and monotonic receipts. Ref at expected commit after crash is accepted only after full object+reflog verification; changed prefix/ABA/other movement parks `planning_ref_foreign_movement`. Missing/corrupt claimed durable state parks `planning_publication_corrupt`; keepalive custody/closure failure parks `planning_candidate_keepalive_invalid`. No re-sign/recompute-from-latest, rebase/reset/force/cherry-pick/adopt-current recovery exists.
...
~~~json
{
  "autosk_flow": {
    ...
    "planning": {
      ...
      "candidate_keepalive_operation": null,
    ...
    "current_artifact": {
      ...
      "snapshot_commit": "...",
      "candidate_keepalive": {
        "ref": "refs/autosk/epics/<epic_ref_key>/candidates/<candidate_identity>",
        "phase": "verified",
...
`candidate_keepalive` lifecycle is closed: candidate_keepalive panel NOT_PASS, narrow NOT_PASS, terminal void or supersession atomically transfers the live candidate ref to its audit ref and records phase=`audit_retained`; verified publication records phase=`released` only after the same audit transfer. Audit-retained refs are valid cleanup state, while a current `prepared|ref_created|verified` ref without disposition is unresolved and blocks cleanup.
...
- Planned implementation запрещён до Published PASS каждого реально созданного артефакта: disposition=pass с verdict либо disposition=waived с signed exact daemon waiver, publication_status=verified, matching planning_publication_op phase=verified, published_commit_oid reachable в live private planning ref first-parent chain, published tree совпадает с recorded candidate tree, а current publication bindings совпадают с current anchor/protocol/runtime/instruction/artifact/alignment identity; только newest verified publication равна live head.
...
| planning_ref_foreign_movement | init_planning_ref or publish_artifact_pass or publish_planning_invalidation according to recorded operation_type; freeze_artifact, synthesize_panel, narrow_review_join or cleanup according to the recorded candidate_keepalive_op, candidate_supersession_op or audit_candidate_housekeeping_op; with no open operation use the recorded detecting gate, only after signed investigation disposition | exact investigation record binds operation type/ID when present, detecting gate, ref/reflog observations and recorded target; ordinary retry/adopt/reset forbidden; unresolved movement permits only separate cancel status operation |
| planning_candidate_keepalive_invalid | freeze_artifact, rebuild_anchor, synthesize_panel, narrow_review_join, record_artifact_pass, current publish step or cleanup according to recorded candidate/operation state | exact candidate identity, keepalive ref/create or audit/release receipt and full closure must be restored without moving another ref; moved foreign ref requires signed investigation or candidate supersession; cleanup remains forbidden until namespace enumeration matches current plus planning.candidate_history |
...
- privileged install/migration establishes sticky/ACL parent topology, unpacks existing refs, pins `gc.packRefs=false`, makes packed-refs helper-owned, and rejects any `refs/autosk/**` packed entry before model launch;

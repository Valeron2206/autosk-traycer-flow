...
| init_planning_ref / freeze_artifact / rebuild_anchor / publish_artifact_pass / publish_planning_invalidation | ref custody ownership/mode/generation drift, helper identity mismatch, packed-refs contains refs/autosk or unauthorized refs/autosk write probe succeeds | retain every candidate_keepalive; human с park.reason=planning_ref_capability_missing; model/ref/cleanup side effects absent |
...
| synthesize_panel | подтверждены findings and candidate_keepalive phase=verified | atomic helper transaction creates audit candidate ref at snapshot commit and deletes live candidate ref, record audit receipt and phase=audit_retained; synthesize_panel |
...
| narrow_review_join | Lead status=done, NOT_PASS/findings and candidate_keepalive phase=verified | atomic helper transaction creates audit candidate ref at snapshot commit and deletes live candidate ref, record audit receipt and phase=audit_retained; narrow_review_join |
...
| publish_artifact_pass | candidate_keepalive ref is absent/moved, create receipt mismatches, or complete commit/tree/blob closure is not connected before planning ref verification | human с park.reason=planning_candidate_keepalive_invalid; object/ref movement отсутствует |
...
| publish_artifact_pass | phase=verified, candidate_keepalive phase=released and effective_target_step=prepare_anchor_impact | read-back same commit/ref/reflog, historical publication projection and release receipt; prepare_anchor_impact |
| publish_artifact_pass | phase=verified, candidate_keepalive phase=released and effective_target_step=select_next | read-back same commit/ref/reflog and release receipt, finalize only missing monotonic projection; select_next |
...
| publish_artifact_pass | phase=voided_before_ref, candidate_keepalive_op phase=audit_retained and recovery_target_step=prepare_anchor_impact | archive immutable terminal publication operation, retain original disposition/publication terminal bindings and pending anchor; prepare_anchor_impact |
...
| cleanup | unresolved current keepalive candidate_keepalive phase=prepared\|ref_created\|verified without audit-retention disposition exists | human с park.reason=planning_candidate_keepalive_invalid; cleanup cannot delete a live closure ref |
| cleanup | все sandboxes/snapshots удалены либо absent, all candidate_keepalive records phase=released\|audit_retained with exact audit refs/receipts, dirty=false | done |
...
| planning_candidate_keepalive_invalid | freeze_artifact, rebuild_anchor or current publish step according to recorded candidate/operation state | exact candidate identity, keepalive ref/create receipt and full closure must be restored without moving another ref; moved foreign ref requires signed investigation or candidate supersession; cleanup remains forbidden |

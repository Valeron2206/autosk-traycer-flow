...
Любой verified candidate, который оставляется без публикации, сначала проходит закрытый `candidate_supersession_op`; общей фразы без durable operation недостаточно. Operation записывается до helper call как `{schema,operation_id,source_step,source_candidate_identity,replacement_intent_digest,keepalive_operation_id,live_ref,audit_ref,snapshot_commit_oid,phase:prepared,audit_receipt:null,created_at_utc}`. `source_step` закрыт значениями `rebuild_anchor|record_artifact_pass|fix_artifact|freeze_artifact`, а phases — `prepared -> audit_transferred -> archived`. Одинаковая candidate identity означает retry и не создаёт supersession; отличающаяся replacement identity/intent требует завершить operation до mint. Helper-транзакция с `reason=superseded` создаёт exact audit ref и удаляет live ref по expected-old. Lost response восстанавливается только по тому же operation ID, exact live-absent/audit-present observation и receipt. `audit_transferred` append'ится в `planning.candidate_history` до очистки current candidate; только archived разрешает целевой шаг и новый candidate. Cleanup сверяет как live, так и audit namespaces с current operation/history.

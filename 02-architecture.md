...
Issue #5 owns the packaged `autosk-flow-ref-custody` component (`src/git/ref-custody-helper.ts`, client and closed protocol). The helper exposes a project-scoped Unix socket; only autoskd's non-inheritable, exact-request-bound daemon capability may authorize init, keepalive, planning-CAS, audit-retain and release requests. Models, same-UID sibling processes and extension subprocesses cannot open an authorized request. Each action-discriminated request binds project/Epic, custody generation, expected refs/OIDs, operation/candidate identity, nonce, exact sanitized reflog producer, update message and prefix checkpoint; its response returns raw appended-entry evidence, before/after counts, observation digest and receipt. The helper is the process that executes every protected `update-ref`. A helper-owned `flock` serializes transactions and releases on process death; request and committed receipt are fsynced around the ref transaction, and a lost response is recovered only by byte-identical journal/nonce replay, so stale-lock recovery never guesses or deletes lock files.

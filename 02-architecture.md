...
Enforceable boundary: a separate-account ref-custody helper is the only writer of `refs/autosk/**` and `logs/refs/autosk/**`. V1 stores those refs in a helper-owned bare custody Git directory outside the user-writable project root and `.git`; project worktrees contain no writable path to the custody common directory. Every ancestor is root/helper-owned and non-writable by project/model accounts; the helper pins device/inode plus owner/mode/ACL, forbids symlink or overlay replacement, and uses descriptor-relative access. Linux and macOS are supported only when bootstrap proves this chain, Unix peer credentials and denied create/rename/pack-refs probes. Windows, network filesystems, writable ancestors or an in-project custody directory fail `planning_ref_capability_missing`.
...
Files-backend packing is closed inside the service-owned custody repository. Bootstrap keeps protected refs loose, pins `gc.packRefs=false`, owns packed-refs and the complete ancestor chain, and rejects any protected packed entry. Only helper-owned maintenance may run there; project `git gc` cannot reach the custody Git directory.
...
`planning_ref_init_op`, `candidate_keepalive_op` и `planning_publication_op` живут в protected namespaced Epic metadata. Init operation хранит selected_base_ref, base-selection policy/authority, bootstrap policy digest, ref storage format, exact reflog producer, closed receipts and golden recovery bindings. Candidate keepalive хранит deterministic candidate ref, snapshot commit/tree, create/release receipts и GC connectivity proof. Publication operation дополнительно хранит typed payload, immutable bindings including the keepalive, complete commit_recipe, exact commit bytes, expected commit OID, effective target и reflog checkpoint. Verified operations атомарно обновляют `planning.last_verified_reflog_tail`, который закрывает движение ref между operations. Terminal records переходят в точные append-only containers: init — `planning.init_history`, released/audit-retained keepalive — `planning.candidate_history`, artifact/invalidation publication — `planning.publication_history`, rebuild — `planning.rebuild_history`. Не дублируются только содержание артефактов и task status. Git ref/object database остаётся source of truth для опубликованной planning line; metadata связывает её с workflow state.
